...
import { modalityFetch } from "./modalityProxy.js";
...
async function upstreamError(res) {
  ...
  return createErrorResult(res.status || HTTP_STATUS.BAD_GATEWAY, msg);
}

// Deepgram: POST binary to {baseUrl}?model=...
async function transcribeDeepgram(cfg, file, model, token, formData, credentials) {
  ...
  const res = await modalityFetch(url, {
    method: "POST",
...
async function transcribeAssemblyAI(cfg, file, model, token, credentials) {
  const auth = buildAuthHeaders(cfg, token);
  const buf = await file.arrayBuffer();
  const up = await modalityFetch("https://api.assemblyai.com/v2/upload", {
    method: "POST", headers: { ...auth, "Content-Type": "application/octet-stream" }, body: buf,
  }, credentials);
  if (!up.ok) return upstreamError(up);
  const { upload_url } = await up.json();

  const sub = await modalityFetch(cfg.baseUrl, {
    method: "POST",
  ...
  while (Date.now() - start < 120_000) {
    ...
    const poll = await modalityFetch(`${cfg.baseUrl}/${id}`, { headers: auth }, credentials);
    if (!poll.ok) continue;
...
async function transcribeNvidia(cfg, file, model, token, credentials) {
  const fd = new FormData();
  fd.append("file", file, file.name || "audio.wav");
  fd.append("model", model);
  const res = await modalityFetch(cfg.baseUrl, { method: "POST", headers: buildAuthHeaders(cfg, token), body: fd }, credentials);
  if (!res.ok) return upstreamError(res);
  const data = await res.json();
...
async function transcribeGemini(cfg, file, model, token, formData, credentials) {
  const buf = await file.arrayBuffer();
...
async function transcribeHuggingFace(cfg, file, model, token, credentials) {
...
async function transcribeOpenAICompatible(cfg, file, model, token, formData, credentials) {
  const fd = new FormData();
  fd.append("file", file, file.name || "audio.wav");
  fd.append("model", model);
...
export async function handleSttCore({ provider, model, formData, credentials, sttConfig }) {
  ...
  try {
    switch (cfg.format) {
      case "deepgram":        return await transcribeDeepgram(cfg, file, model, token, formData, credentials);
      case "assemblyai":      return await transcribeAssemblyAI(cfg, file, model, token, credentials);
      case "nvidia-asr":      return await transcribeNvidia(cfg, file, model, token, credentials);
      case "huggingface-asr": return await transcribeHuggingFace(cfg, file, model, token, credentials);
      case "gemini-stt":      return await transcribeGemini(cfg, file, model, token, formData, credentials);
      default:                return await transcribeOpenAICompatible(cfg, file, model, token, formData, credentials);
    }
  } catch (err) {
