...
export async function POST(request) {
  ...
  // Normalize to array; optional top-level proxyPoolId applies to all imports
  const proxyPoolId = body?.proxyPoolId;
  let accounts;
  ...
  for (let i = 0; i < accounts.length; i++) {
    ...
    try {
      ...
      const created = await createProviderConnection(attachOAuthProxyPoolId({
        provider: "codex",
        authType: "oauth",
        ...item,
      }, item.proxyPoolId || proxyPoolId));
