...
function getStatus(status) {
  ...
  if (isHairouterConfigured(status)) return { label: "Connected", cls: "bg-green-500/10 text-green-600 dark:text-green-400" };
          ...
          <div className="flex items-center gap-3">
            <div className="size-8 flex items-center justify-center shrink-0">
              {tool.image ? (
                ...
                <MdiIcon name={tool.icon} size={28} style={{ color: tool.color }} />
              ) : null}
            </div>
            ...
            <MdiIcon name="chevron_right" size={18} className="text-text-muted shrink-0" />
          </div>
        </div>
      </Card>
    </Link>
