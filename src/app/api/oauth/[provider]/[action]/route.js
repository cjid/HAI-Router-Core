...
function pickProxyPoolId(...sources) {
  for (const src of sources) {
    const id = src?.proxyPoolId;
    if (id && id !== "__none__") return id;
  }
  return null;
}
...
export async function GET(request, { params }) {
  try {
    ...
    if (action === "start-proxy") {
      ...
      const proxyPoolId = searchParams.get("proxy_pool_id");
      ...
      if (result.success && state && codeVerifier && redirectUri) {
        const sessionPayload = { state, codeVerifier, redirectUri, ...(proxyPoolId ? { proxyPoolId } : {}) };
        serverSide = provider === "xai"
          ? registerXaiSession(sessionPayload)
          : registerCodexSession(sessionPayload);
      }
...
export async function POST(request, { params }) {
  try {
    ...
    if (action === "register-session") {
      ...
      let ok = false;
      const proxyPoolId = pickProxyPoolId(body);
      if (provider === "trae") ok = registerTraeSession({ state, ...(proxyPoolId ? { proxyPoolId } : {}) });
      else if (provider === "windsurf") ok = registerWindsurfSession({ state, ...(proxyPoolId ? { proxyPoolId } : {}) });
      else if (provider === "zed") ok = registerZedSession({ state, codeVerifier: body?.codeVerifier, ...(proxyPoolId ? { proxyPoolId } : {}) });
    ...
    if (action === "exchange") {
      ...
      const proxyPoolId = pickProxyPoolId(body, meta);
      const exchangeMeta = proxyPoolId
        ? { ...(meta || {}), proxyPoolId }
        : meta;
      ...
      if (provider === "trae" || provider === "windsurf") {
        ...
        try {
          const tokenData = await exchangeTokens(provider, token, null, null, state, exchangeMeta);
          const connection = await createProviderConnection(attachOAuthProxyPoolId({
            provider,
      ...
      const tokenData = await exchangeTokens(provider, code, redirectUri, codeVerifier, state, exchangeMeta);
    ...
    if (action === "poll") {
      ...
      const proxyPoolId = pickProxyPoolId(body, extraData);
      const pollExtraData = proxyPoolId
        ? { ...(extraData || {}), proxyPoolId }
        : extraData;
      ...
      if (noPkceProviders.includes(provider)) {
        ...
        result = await pollForToken(provider, deviceCode, null, pollExtraData);
        ...
        result = await pollForToken(provider, deviceCode, codeVerifier, pollExtraData);
      } else {
        // Qwen and other PKCE providers
        if (!codeVerifier) {
          return NextResponse.json({ error: "Missing code verifier" }, { status: 400 });
