...
import { connectHttp2Client, isProxyConfigured } from "../utils/http2Connect.js";
...
function encodeHistoryMessage(message) {
  if (message?.role === "tool") {
    const content = textFromContent(message.content) || String(message.content || "");
    if (!content) return null;
    const text = agentString(1, content);
    return agentMessage(3, agentMessage(1, agentMessage(1, text)));
  }
  ...
  if (message?.role === "assistant" && message?.tool_calls?.length) {
    const summary = message.tool_calls
      ...
      .join(", ");
    const text = agentString(1, `[tool_calls: ${summary}]`);
    return agentMessage(2, agentMessage(1, agentMessage(1, text)));
  }
...
export class CursorExecutor extends BaseExecutor {
  ...
  async makeHttp2Request(url, headers, body, signal, proxyOptions = null) {
    const { isGoEngineEnabled } = await import("../../src/lib/goEngine/workerManager.js");
    if (isGoEngineEnabled()) {
      const { goEngineFetch } = await import("../../src/lib/goEngine/goTransport.js");
      const response = await goEngineFetch(url, {
        method: "POST",
        headers,
        body,
        signal,
    ...
    const client = await connectHttp2Client(urlObj.host, proxyOptions);
    const chunks = [];
    let responseHeaders = {};
    let settled = false;
  ...
  async openAgentHttp2Stream(url, headers, signal, proxyOptions = null) {
    const { isGoEngineEnabled } = await import("../../src/lib/goEngine/workerManager.js");
    if (isGoEngineEnabled()) {
      const { goEngineOpenHttp2Stream } = await import("../../src/lib/goEngine/goEngineHttp2.js");
      return goEngineOpenHttp2Stream(url, headers, signal, proxyOptions);
    }

    if (!http2) {
      throw new Error("HTTP/2 is required for Cursor AgentService (endpoint is h2-only)");
    }
    if (isProxyConfigured(proxyOptions) && !proxyOptions?.connectionProxyUrl && proxyOptions?.vercelRelayUrl) {
      throw new Error("Cursor AgentService requires an HTTP CONNECT proxy; Vercel relay cannot proxy HTTP/2 Agent streams");
    }
  ...
  async executeAgent({ model, body, stream, credentials, signal, proxyOptions = null }) {
  ...
  async execute({ model, body, stream, credentials, signal, log, proxyOptions = null }) {
    if (isAgentTextRequest(body)) {
      try {
        return await this.executeAgent({ model, body, stream, credentials, signal, proxyOptions });
      } catch (error) {
        return {
          response: new Response(JSON.stringify({
    ...
    try {
      ...
      const response = (http2 && !shouldForceFetch)
        ? await this.makeHttp2Request(url, headers, transformedBody, signal, proxyOptions)
