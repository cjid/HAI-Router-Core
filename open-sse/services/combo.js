...
import { buildPublicErrorResponse, createInternalError, HAI_CODES } from "../errors/index.js";
import { getCapabilitiesForModel } from "../providers/capabilities.js";
import { extractTextContent } from "../translator/formats/gemini.js";
import { getLaneConfig } from "../concurrency/index.js";
import { isProviderLaneSaturated } from "../concurrency/laneRegistry.js";
import { parseModel } from "./model.js";

const COMBO_TRANSIENT_COOLDOWN_CAP_MS = 1500;

function resolveModelProviderId(modelStr) {
  return parseModel(modelStr).provider || null;
}

function isAdmissionSaturationError(status, errorText, errorCode) {
  if (errorCode === HAI_CODES.queue_timeout || errorCode === HAI_CODES.capacity_exceeded) {
    return true;
  }
  if (status !== 503) return false;
  const lower = (errorText || "").toLowerCase();
  return lower.includes("queue") || lower.includes("capacity") || lower.includes("hai_queue");
}
...
export function reorderByCapabilities(models, required) {
  ...
  const minTier = Math.min(...ranked.map((x) => x.t));
  if (minTier >= 2) return models;
...
export async function handleComboChat({ body, models, handleSingleModel, log, comboName, comboStrategy, comboStickyLimit = 1, autoSwitch = true }) {
  ...
  for (let i = 0; i < rotatedModels.length; i++) {
    ...
    const providerId = resolveModelProviderId(modelStr);

    if (providerId && saturatedProviders.has(providerId)) {
      log.info("COMBO", `Skipping ${modelStr} — provider ${providerId} saturated this combo attempt`);
      lastError = `Provider ${providerId} at capacity`;
      lastStatus = 503;
      continue;
    }

    if (providerId && isProviderLaneSaturated(providerId)) {
      log.info("COMBO", `Provider lane ${providerId} at capacity, skipping ${modelStr}`);
      saturatedProviders.add(providerId);
      lastError = `Provider ${providerId} at capacity`;
      lastStatus = 503;
      continue;
    }
    ...
    try {
      ...
      if (providerId && isAdmissionSaturationError(result.status, errorText, errorCode)) {
        saturatedProviders.add(providerId);
        log.info("COMBO", `Provider ${providerId} admission saturated — skipping remaining same-provider models`);
      }

      // For transient errors (503/502/504), wait briefly before falling through.
      // Skip wait when provider already marked saturated (avoid queue amplification).
      if (cooldownMs && cooldownMs > 0 &&
          (result.status === 503 || result.status === 502 || result.status === 504) &&
          !saturatedProviders.has(providerId)) {
        const waitMs = Math.min(cooldownMs, COMBO_TRANSIENT_COOLDOWN_CAP_MS);
  ...
  const internal = createInternalError({
    statusCode: status,
    upstreamMessage: msg,
    haiCode: HAI_CODES.route_unavailable,
    phase: "combo",
  });
  return buildPublicErrorResponse(internal);
}
...
/** Start panel tasks with bounded concurrency; returns sparse-aligned promise array. */
function startBoundedPanelCalls(items, limit, factory) {
...
export async function handleFusionChat({ body, models, handleSingleModel, log, comboName, judgeModel, tuning }) {
  ...
  const t0 = Date.now();
  const fusionMax = getLaneConfig().fusionMaxParallel || FUSION_DEFAULTS.maxParallel;
  const abortControllers = panel.map(() => new AbortController());
  const calls = startBoundedPanelCalls(panel, fusionMax, (m) =>
  ...
  if (answers.length === 1) {
    log.info("FUSION", `Only ${answers[0].model} succeeded — reusing panel response (no duplicate upstream)`);
