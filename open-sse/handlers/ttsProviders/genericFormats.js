...
// Each handler accepts { baseUrl, apiKey, text, modelId, voiceId, credentials } and returns { base64, format }.
...
async function hyperbolic({ baseUrl, apiKey, text, credentials }) {
  const res = await modalityFetch(baseUrl, {
    method: "POST",
...
async function deepgram({ baseUrl, apiKey, text, modelId, credentials }) {
  const url = new URL(baseUrl);
  url.searchParams.set("model", modelId || "aura-asteria-en");
  const res = await modalityFetch(url.toString(), {
    method: "POST",
...
async function nvidia({ baseUrl, apiKey, text, modelId, voiceId, credentials }) {
  const res = await modalityFetch(baseUrl, {
    method: "POST",
    headers: { "Content-Type": "application/json", "Authorization": `Bearer ${apiKey}` },
...
async function huggingface({ baseUrl, apiKey, text, modelId, credentials }) {
  if (!modelId || modelId.includes("..")) throw new Error("Invalid HuggingFace model ID");
  const res = await modalityFetch(`${baseUrl}/${modelId}`, {
    method: "POST",
    headers: { "Content-Type": "application/json", "Authorization": `Bearer ${apiKey}` },
...
async function fishAudio({ baseUrl, apiKey, text, modelId, voiceId, credentials }) {
  const res = await modalityFetch(baseUrl, {
    method: "POST",
    headers: {
...
async function inworld({ baseUrl, apiKey, text, modelId, voiceId, credentials }) {
  const res = await modalityFetch(baseUrl, {
    method: "POST",
...
async function cartesia({ baseUrl, apiKey, text, modelId, voiceId, credentials }) {
  const res = await modalityFetch(baseUrl, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
...
async function playht({ baseUrl, apiKey, text, modelId, voiceId, credentials }) {
...
async function coqui({ baseUrl, text, voiceId, credentials }) {
  const res = await modalityFetch(baseUrl, {
    method: "POST",
...
async function tortoise({ baseUrl, text, voiceId, credentials }) {
  const res = await modalityFetch(baseUrl, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
async function openaiCompat({ baseUrl, apiKey, text, modelId, voiceId, credentials }) {
