...
import { modalityFetch, resolveModalityProxyOptions } from "./modalityProxy.js";
...
export async function handleImageGenerationCore({
  ...
  if (adapter.useExecutor && adapter.executeViaExecutor) {
    try {
      ...
      if (binaryOutput) {
        ...
        if (!b64 && first?.url) {
          try { b64 = await urlToBase64(first.url, credentials); } catch {}
        }
        if (b64) {
  ...
  const proxyOptions = resolveModalityProxyOptions(credentials);
  let providerResponse;
  try {
    providerResponse = await modalityFetch(url, {
      method: "POST",
      headers,
  ...
  if (
    ...
    if (newCredentials?.accessToken || newCredentials?.apiKey) {
      ...
      try {
        ...
        providerResponse = await modalityFetch(retryUrl, {
          method: "POST",
