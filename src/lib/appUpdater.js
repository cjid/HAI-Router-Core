...
// Collect PIDs of all HAI-Router-related processes (excluding current)
  ...
  if (platform === "win32") {
    try {
      ...
      lines.forEach(line => {
        ...
        // Match anything running from HAI-Router install dir or wrapper cli.js
        const isAppProcess = lower.includes("hairouter") ||
    ...
    try {
      ...
      output.split("\n").forEach(line => {
        const isAppProcess = line.includes("hairouter") ||
...
function ensureRuntimeUpdater(bundledPath) {
  try {
    ...
    const runtimeDir = path.join(getRuntimeDataDir(), "runtime", "updater");
