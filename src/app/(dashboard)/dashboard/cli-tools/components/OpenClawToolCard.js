...
import {
  ...
  pickProviderBlock,
  isHairouterConfigured,
} from "./hairouterCliHelpers";
...
export default function OpenClawToolCard({
  ...
  const applyProcess = useAsyncProcess({ loadingState: PROCESS_STATE.SAVING });
  const resetProcess = useAsyncProcess({ loadingState: PROCESS_STATE.LOADING });
  ...
  const hairouterProvider = pickProviderBlock(openclawStatus?.settings?.models?.providers);
  const currentBaseUrl = hairouterProvider?.baseUrl || "";

  const getConfigStatus = () => {
    if (!openclawStatus?.installed) return null;
    const currentProvider = pickProviderBlock(openclawStatus.settings?.models?.providers);
  ...
  useEffect(() => {
    if (openclawStatus?.installed && !hasInitializedModel.current) {
      ...
      const provider = pickProviderBlock(openclawStatus.settings?.models?.providers);
      if (provider) {
        const primaryModel = openclawStatus.settings?.agents?.defaults?.model?.primary;
        if (primaryModel && isHairouterPrefixedModel(primaryModel)) {
          setSelectedModel(stripHairouterModelPrefix(primaryModel) || "");
        }
  ...
  const handleApplySettings = () => applyProcess.run(async () => {
    setMessage(null);
    try {
      const keyToUse = selectedApiKey?.trim()
        || (apiKeys?.length > 0 ? apiKeys[0].key : null)
        || (!cloudEnabled ? "sk_hairouter" : null);
  ...
  const handleResetSettings = () => resetProcess.run(async () => {
    setMessage(null);
    try {
  ...
  const getManualConfigs = () => {
    const keyToUse = (selectedApiKey && selectedApiKey.trim())
      ? selectedApiKey
      : (!cloudEnabled ? "sk_hairouter" : "<API_KEY_FROM_DASHBOARD>");

    const settingsContent = {
      agents: {
        defaults: {
          model: {
            primary: hairouterModelId(selectedModel || "provider/model-id"),
          },
        },
      },
      models: {
  ...
  return (
    <Card padding="xs" className="overflow-hidden">
      <div className="flex items-start justify-between gap-3 hover:cursor-pointer sm:items-center" onClick={onToggle}>
        ...
        <MdiIcon name={"expand_more"} size={20} className={`text-text-muted transition-transform ${isExpanded ? "rotate-180" : ""}`} />
      </div>
        ...
        <div className="mt-4 pt-4 border-t border-border flex flex-col gap-4">
          {checkingOpenclaw && (
            <div className="flex items-center gap-2 text-text-muted">
              <MdiIcon name="progress_activity" size={18} spin className="animate-spin" />
            ...
            <div className="flex flex-col gap-4">
              <div className="flex flex-col gap-3 p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
                <div className="flex items-start gap-3">
                  <MdiIcon name="warning" size={18} className="text-yellow-500" />
                  <div className="flex-1">
                    <p className="font-medium text-yellow-600 dark:text-yellow-400">Open Claw CLI not detected locally</p>
                    <p className="text-sm text-text-muted">Manual configuration is still available if HAI-Router is deployed on a remote server.</p>
                  </div>
                </div>
                <div className="flex items-center gap-2 pl-9">
                  <Button variant="secondary" size="sm" onClick={() => setShowManualConfigModal(true)} className="!bg-yellow-500/20 !border-yellow-500/40 !text-yellow-700 dark:!text-yellow-300 hover:!bg-yellow-500/30">
                    <MdiIcon name="content_copy" size={18} className="mr-1" />
                    Manual Config
                  </Button>
                </div>
              </div>
            ...
            <>
              <div className="flex flex-col gap-2">
                ...
                <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr] sm:items-center sm:gap-2">
                  ...
                  <MdiIcon name="arrow_forward" size={14} className="hidden text-text-muted sm:inline" />
                  <BaseUrlSelect
                ...
                <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr_auto] sm:items-center sm:gap-2">
                  ...
                  <div className="relative w-full min-w-0">
                    ...
                    {selectedModel && <button onClick={() => setSelectedModel("")} className="absolute right-1 top-1/2 -translate-y-1/2 p-0.5 text-text-muted hover:text-red-500 rounded transition-colors" title="Clear"><MdiIcon name="close" size={14} /></button>}
                  </div>
                ...
                {(openclawStatus.agents || []).filter(a => a.agentDir).map((agent) => (
                  <div key={agent.id} className="flex items-center gap-2 pl-4">
                    ...
                    <div className="relative w-full min-w-0">
                      ...
                      />
                      {agentModels[agent.id] && <button onClick={() => setAgentModels(prev => ({ ...prev, [agent.id]: "" }))} className="absolute right-1 top-1/2 -translate-y-1/2 p-0.5 text-text-muted hover:text-red-500 rounded transition-colors" title="Clear"><MdiIcon name="close" size={14} /></button>}
                    </div>
              ...
              {message && (
                <div className={`flex items-center gap-2 px-2 py-1.5 rounded text-xs ${message.type === "success" ? "bg-green-500/10 text-green-600" : "bg-red-500/10 text-red-600"}`}>
                  <MdiIcon name={message.type === "success" ? "check_circle" : "error"} size={14} />
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                <Button
                  ...
                  processState={applyProcess.processState}
                ...
                <Button
                  ...
                  disabled={!isHairouterConfigured(openclawStatus)}
                  processState={resetProcess.processState}
                  processLabels={PROCESS_LABEL_PRESETS.reset}
                  icon="restore"
                />
                <Button variant="ghost" size="sm" onClick={() => setShowManualConfigModal(true)}>
                  <MdiIcon name="content_copy" size={14} className="mr-1" />Manual Config
                </Button>
              </div>
            </>
          )}
