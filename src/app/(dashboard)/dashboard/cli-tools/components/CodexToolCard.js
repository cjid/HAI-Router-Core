...
import { CLI_PROVIDER_SLUG, CLI_GATEWAY_DISPLAY_NAME } from "@/shared/constants/cliIdentity.js";
...
export default function CodexToolCard({ tool, isExpanded, onToggle, baseUrl, apiKeys, activeProviders, cloudEnabled, initialStatus, tunnelEnabled, tunnelPublicUrl, tailscaleEnabled, tailscaleUrl }) {
  ...
  const handleApplySettings = () => applyProcess.run(async () => {
    setMessage(null);
    try {
      // Use sk_hairouter for localhost if no key, otherwise use selected key
  ...
  const getManualConfigs = () => {
    ...
    const configContent = `# HAI-Router Configuration for Codex CLI
model = "${selectedModel}"
model_provider = "${CLI_PROVIDER_SLUG}"

[model_providers.${CLI_PROVIDER_SLUG}]
name = "${CLI_GATEWAY_DISPLAY_NAME}"
base_url = "${getEffectiveBaseUrl()}"
wire_api = "responses"

[model_providers.${CLI_PROVIDER_SLUG}.http_headers]
            ...
            <>
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                <Button
                  ...
                  disabled={(!selectedApiKey && (cloudEnabled && apiKeys.length > 0)) || !selectedModel}
