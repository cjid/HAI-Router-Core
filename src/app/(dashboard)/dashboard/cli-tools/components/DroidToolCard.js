...
import { isHairouterConfigured, isDroidHairouterModel, HAIROUTER_DROID_PREFIX, LEGACY_DROID_PREFIX } from "./hairouterCliHelpers";
...
export default function DroidToolCard({
  ...
  const currentBaseUrl = droidStatus?.settings?.customModels?.find(isDroidHairouterModel)?.baseUrl || "";

  const getConfigStatus = () => {
    if (!droidStatus?.installed) return null;
    const currentConfig = droidStatus.settings?.customModels?.find(isDroidHairouterModel);
  ...
  useEffect(() => {
    if (droidStatus?.installed && !hasInitializedModel.current) {
      ...
      const existingModels = (droidStatus.settings?.customModels || [])
        .filter(isDroidHairouterModel)
        .sort((a, b) => (a.index || 0) - (b.index || 0))
        .map(m => m.model);
      if (existingModels.length > 0) {
        setModelList(existingModels);
      } else {
        const legacy = droidStatus.settings?.customModels?.find(m => m.id === `${LEGACY_DROID_PREFIX}-0`);
        if (legacy?.model) {
          ...
          {!checkingDroid && droidStatus?.installed && (
            <>
              <div className="flex flex-col gap-2">
                ...
                {/* Current configured */}
                {droidStatus?.settings?.customModels?.find(isDroidHairouterModel)?.baseUrl && (
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                ...
                <Button
                  ...
                  disabled={!isHairouterConfigured(droidStatus)}
