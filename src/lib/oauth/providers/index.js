// Ensure OAuth outbound uses proxy-aware transport (without loading the full open-sse bundle).
import "open-sse/utils/proxyFetch.js";

import { generatePKCE } from "../utils/pkce.js";
import { extractCodexAccountInfo, fetchKiroProfileArn } from "../providerHelpers.js";
import { resolveOAuthProxyOptions } from "@/lib/network/connectionProxy.js";
...
export async function exchangeTokens(providerName, code, redirectUri, codeVerifier, state, meta) {
  const proxyOptions = await resolveOAuthProxyOptions(meta?.proxyPoolId);
  return runWithOAuthProxy(proxyOptions, async () => {
    const provider = getProvider(providerName);
    const config = provider.prepareConfig
      ? await provider.prepareConfig(provider.config, meta || {})
      : provider.config;
...
export async function requestDeviceCode(providerName, codeChallenge, options) {
  const proxyOptions = await resolveOAuthProxyOptions(options?.proxyPoolId);
  return runWithOAuthProxy(proxyOptions, async () => {
    const provider = getProvider(providerName);
    if (provider.flowType !== "device_code") {
...
export async function pollForToken(providerName, deviceCode, codeVerifier, extraData) {
  const proxyOptions = await resolveOAuthProxyOptions(extraData?.proxyPoolId);
  return runWithOAuthProxy(proxyOptions, async () => {
    const provider = getProvider(providerName);
    if (provider.flowType !== "device_code") {
      throw new Error(`Provider ${providerName} does not support device code flow`);
