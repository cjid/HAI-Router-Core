...
export function releaseConnection(connectionId) {
  ...
  invalidateConnectionEgress(connectionId);
...
export async function checkAndRefreshToken(provider, credentials, options = {}) {
  ...
  if (force || _shouldRefreshCredentials(provider, creds)) {
    ...
    if (newCreds?.accessToken || newCreds?.apiKey || newCreds?.copilotToken) {
      ...
      _refreshProjectId(provider, creds.connectionId, creds.accessToken, enrichProxyOptions(buildProxyOptionsFromCredentials(creds), { providerId: provider, connectionId: creds.connectionId }));
    }
  }
  ...
  if (provider === "github") {
    ...
    if (!copilotToken || remaining < TOKEN_EXPIRY_BUFFER_MS) {
      ...
      const proxyOpts = enrichProxyOptions(buildProxyOptionsFromCredentials(creds), { providerId: provider, connectionId: creds.connectionId });
      const copilotTokenResult = await _refreshCopilotToken(creds.accessToken, log, proxyOpts);
