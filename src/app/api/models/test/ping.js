...
/** Shared chat probe — long enough for reliable input-token estimation when upstream omits usage. */
export const MODEL_TEST_CHAT_MESSAGE = "Reply with one word: ok";
...
export async function pingModelByKind(model, kind, baseUrl = `http://127.0.0.1:${process.env.PORT || UPDATER_CONFIG.appPort}`) {
  ...
  if (kind === "embedding") {
    const res = await loopbackFetch(`${baseUrl}/api/v1/embeddings`, {
      method: "POST",
      headers,
  ...
  if (kind === "image") {
    const res = await loopbackFetch(`${baseUrl}/api/v1/images/generations`, {
      method: "POST",
      headers,
  ...
  if (kind === "stt") {
    ...
    const res = await loopbackFetch(`${baseUrl}/api/v1/audio/transcriptions`, {
      method: "POST",
  ...
  const res = await loopbackFetch(`${baseUrl}/api/v1/chat/completions`, {
    method: "POST",
    headers,
    body: JSON.stringify({
      model,
      ...
      max_tokens: 1024,
      stream: false,
      messages: [{ role: "user", content: MODEL_TEST_CHAT_MESSAGE }],
    }),
    signal: AbortSignal.timeout(15000),
  });
  const latencyMs = Date.now() - start;
