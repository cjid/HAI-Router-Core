...
import { Card, Button, Badge, Input, Modal, ProviderDetailSkeleton, OAuthModal, KiroOAuthWrapper, CursorAuthModal, IFlowCookieModal, GitLabAuthModal, Toggle, Select, EditConnectionModal, NoAuthProxyCard, ConfirmModal, ProviderIcon } from "@/shared/components";
...
import { modelTestFeedbackMs } from "../components/modelTestVisual";
...
export default function ProviderDetailPage() {
  ...
  const [modelTestAlert, setModelTestAlert] = useState(null);
  const [testingModelIds, setTestingModelIds] = useState(() => new Set());
  const modelTestResetTimers = useRef({});
  ...
  const fetchConnections = useCallback(async () => {
    try {
      ...
      if (connectionsRes.ok) {
        ...
        const states = connectionsData.providerStates || {};
        setProviderEnabled(states[providerId] !== false);
      }
      if (proxyPoolsRes.ok) {
  ...
  const handleToggleProviderEnabled = async (newActive) => {
    setProviderEnabled(newActive);
    try {
      await fetch("/api/providers/state", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ providerId, isEnabled: newActive }),
      });
    } catch (error) {
      console.log("Error toggling provider state:", error);
      setProviderEnabled(!newActive);
    }
  };
  ...
  useEffect(() => {
    ...
    fetchSuggestedModels(fetcher).then((models) => {
      startTransition(() => setSuggestedModels(models));
    });
  }, [providerId]);
    ...
    >
      <div className="flex flex-col gap-3">
        <div className="flex flex-col">
          ...
          >
            <MdiIcon name="sync_alt" size={18} className="text-text-muted" />
          ...
          <button
            ...
            className="flex items-center gap-2 rounded-lg px-3 py-2 text-left transition-colors hover:bg-black/[0.04] dark:hover:bg-white/[0.04] disabled:cursor-not-allowed disabled:opacity-50"
          >
            <MdiIcon name="link_off" size={18} className="text-text-muted" />
  ...
  const handleTestModel = async (modelId) => {
    ...
    const modelLabel = `${providerStorageAlias}/${modelId}`;

    if (modelTestResetTimers.current[modelId]) {
      clearTimeout(modelTestResetTimers.current[modelId]);
      delete modelTestResetTimers.current[modelId];
    }
    ...
    setModelTestAlert(null);

    const scheduleReset = (result) => {
      if (modelTestResetTimers.current[modelId]) {
        clearTimeout(modelTestResetTimers.current[modelId]);
      }
      modelTestResetTimers.current[modelId] = setTimeout(() => {
        setModelTestResults((prev) => {
          if (prev[modelId] !== result) return prev;
          const next = { ...prev };
          delete next[modelId];
          return next;
        });
        delete modelTestResetTimers.current[modelId];
      }, modelTestFeedbackMs(result));
    };
    try {
  ...
  useEffect(() => () => {
    Object.values(modelTestResetTimers.current).forEach((timerId) => clearTimeout(timerId));
  }, []);
  ...
  const renderModelsSection = () => {
    ...
    return (
      <ModelsTableSection
        ...
        onTest={connections.length > 0 || isFreeNoAuth ? handleTestModel : undefined}
        onDisable={handleDisableModel}
        onDeleteCustom={(modelId) => handleDeleteCustomModel(modelId, "llm", providerStorageAlias)}
        ...
        modelTestAlert={modelTestAlert}
        onDismissModelTestAlert={() => setModelTestAlert(null)}
      />
    );
  };
  ...
  const headerIconSrc = (isOpenAICompatible || isAnthropicCompatible) && providerInfo?.baseUrl
    ? null
    : (() => {
  ...
  return (
    <div className="flex min-w-0 flex-col gap-6 px-1 sm:gap-8 sm:px-0">
      ...
      <div className="min-w-0">
        ...
        >
          <MdiIcon name="arrow_back" size={18} />
          Back to Providers
        </Link>
        <div className="flex min-w-0 items-center gap-3 sm:gap-4">
          ...
          >
            <ProviderIcon
              ...
              completionBaseUrl={providerInfo.baseUrl}
              compatibility={isAnthropicCompatible ? "anthropic" : isOpenAICompatible ? "openai" : undefined}
          ...
          <div className="min-w-0">
            ...
            <p className="text-text-muted">
              {isFreeNoAuth
                ? (providerEnabled ? "Ready to use · No authentication required" : "Disabled")
      ...
      {providerInfo.notice?.text && !providerInfo.deprecated && (
        <div className="flex flex-col gap-2 rounded-lg border border-blue-500/30 bg-blue-500/10 px-3 py-2 sm:flex-row sm:items-center">
          <MdiIcon name="info" size={16} className="text-blue-500 shrink-0" />
      ...
      {isFreeNoAuth ? (
        <NoAuthProxyCard
          ...
          onToggle={handleToggleProviderEnabled}
        />
      ) : (
        <Card>
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div className="flex items-center gap-3">
                <div className="inline-flex items-center justify-center w-9 h-9 rounded-full bg-primary/10 text-primary shrink-0">
                  <MdiIcon name={isOAuth ? "lock" : "key"} size={18} />
                </div>
                <div className="min-w-0">
