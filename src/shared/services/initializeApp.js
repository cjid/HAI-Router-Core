  ...
  try {
    ...
    if (!g.signalHandlersRegistered) {
      const runShutdown = async () => {
        ...
        killCloudflared();
        try {
          const { shutdownGoEngine } = await import("@/lib/goEngine/goEngineManager.js");
          await shutdownGoEngine();
        } catch (e) {
          console.log("[GoEngine] shutdown failed:", e?.message || e);
        }
        process.exit(0);
      };
      const onSignal = () => {
        runShutdown().catch(() => process.exit(1));
      };
...
async function runHeavyStartup() {
  ...
  import("@/lib/goEngine/goEngineManager.js")
    .then(async ({ ensureGoEngineStarted }) => {
      const { isGoEngineEnabled } = await import("@/lib/goEngine/workerManager.js");
      if (!isGoEngineEnabled()) return;
      return ensureGoEngineStarted();
    })
    .catch((e) => console.log("[GoEngine] worker start failed:", e.message));
}
...
async function autoStartMitm(settings) {
  ...
  try {
    ...
    await startMitm(activeKey?.key || DEFAULT_LOCAL_API_KEY, password);
