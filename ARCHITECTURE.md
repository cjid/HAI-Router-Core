# HAI-Router Architecture

_Last updated: 2026-02-06_

## Executive Summary

HAI-Router is a local AI routing gateway and dashboard built on Next.js.
...
- Next.js (Node) control plane: client `/v1/*`, routing, translation, combo/account fallback, usage, request details, proxy policy
- Go Engine worker (`go-engine/bin/hai-worker`): provider-facing HTTP transport spawned/managed by Node
- Shared SSE/routing core in `src/sse/*` + `open-sse/*` handles provider execution orchestration, translation, streaming, fallback, and usage
...
flowchart LR
    ...
    subgraph Router[HAI-Router Local Process]
...
- `src/lib/localDb.js`
- file: `${DATA_DIR}/db/data.sqlite` (canonical; legacy `~/.9router/` migrated on first run)
- entities: providerConnections, providerNodes, modelAliases, combos, apiKeys, settings, pricing

Usage DB:

- `src/lib/usageDb.js`
- files: under resolved `DATA_DIR` (usage + logs; legacy `~/.9router/` migrated when present)
...
- main state: `${DATA_DIR}/db/data.sqlite` (SQLite; legacy `~/.9router/` migrated when present)
- usage stats: `${DATA_DIR}/usage.json` (or legacy path during migration)
...
flowchart LR
    ...
    subgraph ContainerOrProcess[HAI-Router Runtime]
        Next[Next.js Server\nPORT=20128]
...
1. `usageDb` paths follow resolved `DATA_DIR`; legacy `~/.9router/` is migrated automatically when present.
...
- Build from source: `npm run build` (repo root)
- Build Docker image: `docker build -t hairouter-app .` (repo root; see `DOCKER.md`)
