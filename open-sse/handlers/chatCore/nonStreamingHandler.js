...
import { buildRequestDetail, extractRequestConfig, extractUsageFromResponse, saveUsageStats, formatDoneLine, resolvePersistedUsage } from "./requestDetail.js";
import { classifyTurnFromChatCompletion } from "@/shared/utils/llmTurnType.js";
...
export async function handleNonStreamingResponse({ providerResponse, provider, model, sourceFormat, targetFormat, body, stream, translatedBody, finalBody, requestStartTime, connectionId, apiKey, clientRawRequest, onRequestSuccess, reqLogger, toolNameMap, customToolNames, trackDone, appendLog, pxpipe, reqTag, log, requestKind = null }) {
  ...
  const turnType = classifyTurnFromChatCompletion(translatedResponse || responseBody);
  const responseContent = translatedResponse?.choices?.[0]?.message?.content || translatedResponse?.content || "";
  const responseContentLength = typeof responseContent === "string" ? responseContent.length : 0;
  saveUsageStats({
    provider, model, tokens: usage, connectionId, apiKey,
    endpoint: clientRawRequest?.endpoint, silent: true, turnType, requestKind,
    body,
    contentLength: responseContentLength,
    hasContent: responseContentLength > 0,
  });

  const totalLatency = Date.now() - requestStartTime;
  const persistedUsage = resolvePersistedUsage({
    tokens: usage,
    requestKind,
    body,
    contentLength: responseContentLength,
  ...
  saveRequestDetail(buildRequestDetail({
    ...
    usageStatus: persistedUsage.usageStatus,
    usageSource: persistedUsage.usageSource,
