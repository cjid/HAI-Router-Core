...
const HAIROUTER_CUSTOM_PREFIX = "custom:HAI-Router";
const LEGACY_CUSTOM_PREFIX = "custom:9Router";

const isHairouterCustomModel = (m) =>
  m?.id?.startsWith(HAIROUTER_CUSTOM_PREFIX) || m?.id?.startsWith(LEGACY_CUSTOM_PREFIX);

const hasHairouterConfig = (settings) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some(isHairouterCustomModel);
};
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      installed: true,
      settings,
      hasHairouter: hasHairouterConfig(settings),
...
export async function POST(request) {
  try {
    ...
    settings.customModels = settings.customModels.filter((m) => !isHairouterCustomModel(m));
