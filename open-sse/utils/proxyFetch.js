...
/** Decide whether a proxy transport failure may fall back to direct egress. */
export function resolveProxyFailureAction(proxyOptions, proxyError) {
  if (proxyOptions?.strictProxy === true) {
    return {
      action: "throw",
      error: new Error(`[ProxyFetch] Proxy required but failed (strictProxy=true): ${proxyError?.message || proxyError}`),
    };
  }
  return { action: "fallback_direct" };
}
...
/**
 * Create proxy dispatcher lazily (undici-compatible). Promise-cached to avoid races;
 * LRU touch on hit; close evicted agents.
 */
async function getDispatcher(proxyUrl) {
  ...
  if (proxyDispatchers.has(normalized)) {
    const agent = proxyDispatchers.get(normalized);
  ...
  if (!proxyDispatcherPromises.has(normalized)) {
    ...
    proxyDispatcherPromises.set(normalized, promise);
  }

  try {
    const agent = await proxyDispatcherPromises.get(normalized);
...
export function resetProxyDispatchersForTests() {
  proxyDispatchers.clear();
  proxyDispatcherPromises.clear();
}
...
export async function proxyAwareFetch(url, options = {}, proxyOptions = null) {
  ...
  // Vitest harness: unit tests stub global.fetch; skip live worker IPC unless integration flag set.
  if (process.env.VITEST === "true" && process.env.HAI_GO_ENGINE_INTEGRATION !== "1") {
    return globalThis.fetch(url, options);
  }

  const { canUseGoEngineTransport, goEngineFetch } = await import("../../src/lib/goEngine/goTransport.js");
  if (canUseGoEngineTransport(proxyOptions)) {
    return await goEngineFetch(url, options, proxyOptions);
  }

  const err = new Error("Go provider engine unavailable (set HAI_GO_ENGINE=0 disables canonical transport)");
 ...
 * Opt-in: set HAI_PATCH_GLOBAL_FETCH=1. When Go engine is canonical (default), global patch stays off.
 */
async function patchedFetch(url, options = {}) {
  return proxyAwareFetch(url, options, null);
}

function shouldPatchGlobalFetch() {
  const explicit = process.env.HAI_PATCH_GLOBAL_FETCH;
...
if (shouldPatchGlobalFetch() && globalThis.fetch !== patchedFetch) {
