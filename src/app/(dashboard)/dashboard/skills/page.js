...
function CopyButton({ value, label = "Copy link" }) {
  ...
  return (
    <button
      ...
      title={value}
    >
      <MdiIcon name={copied ? "check" : "content_copy"} size={12} />
      {copied ? "Copied!" : label}
    </button>
  );
}
...
function SkillRow({ skill }) {
  ...
  return (
    ...
    >
      ...
      >
        <MdiIcon name={skill.icon} size={18} />
      </div>
      ...
      <div className="min-w-0 flex-1">
        ...
        >
          {url}
          <MdiIcon name="open_in_new" size={18} className="text-[12px]" />
        </a>
      </div>
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      <Card padding="md">
        ...
        <div className="px-3 py-2 rounded bg-surface-2 font-mono text-[12px] text-text-main">
          Read this skill and use it: {getSkillRawUrl(SKILLS_ENTRY_ID)}
        </div>
      </Card>
