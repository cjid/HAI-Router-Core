...
export default function MitmServerCard({ apiKeys, cloudEnabled, onStatusChange }) {
  ...
  return (
    <>
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex min-w-0 flex-wrap items-center gap-2">
              <MdiIcon name="security" size={20} className="text-primary" />
            ...
            <div className="flex flex-wrap items-center gap-1 text-xs text-text-muted" data-i18n-skip="true">
              {[
                ...
                <span key={label} className={`flex items-center gap-0.5 px-1.5 py-0.5 rounded ${ok ? "text-green-600" : "text-text-muted"}`}>
                  <MdiIcon name={ok ? "check_circle" : "cancel"} size={12} />
                  {label}
                </span>
              ))}
            </div>
          ...
          <div className="px-2 py-2 rounded-lg bg-surface/50 border border-border/50 flex flex-col gap-2">
            <p className="text-[11px] text-text-muted leading-relaxed">
              <span className="font-medium text-text-main">Purpose:</span> Use Antigravity IDE & GitHub Copilot → with ANY provider/model from HAI-Router
            </p>
            <p className="text-[11px] text-text-muted leading-relaxed">
              <span className="font-medium text-text-main">How it works:</span> Antigravity/Copilot IDE request → DNS redirect to localhost:443 → MITM proxy intercepts → HAI-Router → response to Antigravity/Copilot
            </p>
          </div>
          ...
          <div className="flex flex-col gap-2">
            <div className="grid gap-1 sm:grid-cols-[8rem_auto_1fr] sm:items-center sm:gap-2">
              <span className="text-xs font-semibold text-text-main sm:text-right sm:text-sm">HAI-Router Base URL</span>
            ...
            {!isRunning && (
              <div className="grid gap-1 sm:grid-cols-[8rem_auto_1fr] sm:items-center sm:gap-2">
                ...
                <input
                  ...
                  placeholder={cloudEnabled ? "Enter or pick API key" : "sk_hairouter (default)"}
          ...
          <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-center" data-i18n-skip="true">
            {status?.certExists && !status?.certTrusted && (
              ...
              >
                <MdiIcon name="verified_user" size={16} />
                Trust Cert
              </button>
            )}
            {isRunning ? (
              ...
              >
                <MdiIcon name="stop_circle" size={16} />
                Stop Server
              </button>
            ) : (
              <button
              ...
              >
                <MdiIcon name="play_circle" size={16} />
                Start Server
              </button>
            )}
            {isRunning && (
          ...
          {actionError && (
            <div className="flex items-start gap-2 px-2 py-1.5 rounded text-xs bg-red-500/10 text-red-600 dark:text-red-400 border border-red-500/20">
              <MdiIcon name="error" size={14} className="mt-0.5 shrink-0" />
          ...
          {serverIsWindows && !isAdmin && (
            <div className="flex items-center gap-2 px-2 py-1.5 rounded text-xs bg-red-500/10 text-red-600 border border-red-500/20">
              <MdiIcon name="shield_lock" size={14} />
              <span>Administrator required — restart HAI-Router as Administrator to use MITM</span>
            </div>
          )}
        </div>
      </Card>
      ...
      {/* Password Modal */}
      {showPasswordModal && (
        <div className={mainPanelOverlayClass("flex items-center justify-center bg-black/50 backdrop-blur-sm")}>
          <div className="mx-4 flex w-full max-w-sm flex-col gap-4 rounded-xl border border-border bg-surface p-5 shadow-xl sm:p-6">
            ...
            <div className="flex items-start gap-3 p-3 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
              <MdiIcon name="warning" size={20} className="text-yellow-500" />
