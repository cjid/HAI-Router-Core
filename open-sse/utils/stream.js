...
import { extractUsage, mergeUsage, hasValidUsage, estimateUsage, estimateInputTokens, estimateOutputTokens, estimateContentLengthFromBytes, formatUsage, logUsage, addBufferToUsage, filterUsageForFormat, COLORS } from "./usageTracking.js";
import { parseSSELine, hasValuableContent, hasMeaningfulTokenContent, fixInvalidId, formatSSE } from "./streamHelpers.js";
...
export function createSSEStream(options = {}) {
  ...
  const estimateBody = usageEstimateBody || body;
  ...
  const markMeaningfulToken = (parsed, format) => {
    if (ttftAt || !parsed) return;
    if (!hasMeaningfulTokenContent(parsed, format)) return;
    ttftAt = Date.now();
    requestTiming?.markClientFirstToken?.(ttftAt);
  };

  // Usage/logging tail, callable from transform() as well as flush(): a client that
  // closes right after the terminal event cancels the reader, and flush() never runs.
  // Also exposed as transformStream.finalize() for abort/disconnect paths.
  const finalizeStream = (terminalContext = null) => {
    if (finalized) return;
    finalized = true;

    const isPassthrough = mode === STREAM_MODE.PASSTHROUGH;
    const usageFormat = isPassthrough ? FORMATS.OPENAI : sourceFormat;
    let finalUsage = isPassthrough ? usage : state?.usage;
    const hadProviderUsage = hasValidUsage(finalUsage) && !finalUsage?.estimated;
    const isTerminalAbort = terminalContext?.kind && terminalContext.kind !== "complete";
    let effectiveContentLength = totalContentLength;
    if (effectiveContentLength === 0 && terminalContext?.totalBytes > 0) {
      effectiveContentLength = estimateContentLengthFromBytes(terminalContext.totalBytes);
    }
    if (effectiveContentLength === 0 && sawToolCalls && terminalContext?.totalBytes > 0) {
      effectiveContentLength = Math.max(1, Math.floor(terminalContext.totalBytes / 12));
    }

    const hadStreamBytes = (terminalContext?.totalBytes ?? 0) > 0 || (terminalContext?.chunkCount ?? 0) > 0;
    if (!hasValidUsage(finalUsage) && (effectiveContentLength > 0 || estimateBody || hadStreamBytes)) {
      finalUsage = estimateUsage(estimateBody, effectiveContentLength, usageFormat);
      if (isPassthrough) usage = finalUsage; else state.usage = finalUsage;
    } else if (!hasValidUsage(finalUsage) && estimateBody && isTerminalAbort) {
      const inputOnly = estimateInputTokens(estimateBody);
      if (inputOnly > 0) {
        finalUsage = formatUsage(inputOnly, estimateOutputTokens(effectiveContentLength), usageFormat);
        if (isPassthrough) usage = finalUsage; else state.usage = finalUsage;
      }
    } else if (!hasValidUsage(finalUsage) && hadStreamBytes && isTerminalAbort) {
      finalUsage = estimateUsage(null, effectiveContentLength, usageFormat);
      if (isPassthrough) usage = finalUsage; else state.usage = finalUsage;
    }
    ...
    if (onStreamComplete) {
      const finishReason = streamFinishReason || state?.finishReason || null;
      const hasToolCalls = sawToolCalls || (state?.toolCalls?.size > 0);
      onStreamComplete({
        ...
        ...(terminalContext || {}),
        kind: terminalContext?.kind || "complete",
        hadProviderUsage,
    ...
    transform(chunk, controller) {
      ...
      for (const line of lines) {
        ...
        if (mode === STREAM_MODE.PASSTHROUGH) {
              ...
              if (Array.isArray(delta?.tool_calls) && delta.tool_calls.length > 0) {
                ...
                for (const tc of delta.tool_calls) {
                  ...
                  if (typeof args === "string" && args) {
                    totalContentLength += args.length;
          ...
          if (trimmed.startsWith("data:") && trimmed.slice(5).trim() !== "[DONE]") {
            try {
              markMeaningfulToken(JSON.parse(trimmed.slice(5).trim()), FORMATS.OPENAI);
            } catch { /* skip non-json */ }
          }
        ...
        if (parsed.choices?.[0]?.delta?.tool_calls?.length) {
          sawToolCalls = true;
        }
        if (parsed.choices?.[0]?.finish_reason) {
          streamFinishReason = parsed.choices[0].finish_reason;
        ...
        if (translated?.length > 0) {
          for (const item of translated) {
            ...
            if (item.choices?.[0]?.finish_reason) {
              streamFinishReason = item.choices[0].finish_reason;
            }
            ...
            reqLogger?.appendConvertedChunk?.(output);
            markMeaningfulToken(item, sourceFormat);
            controller.enqueue(sharedEncoder.encode(output));
            sseEmittedCount++;
          }
        }
  ...
  const finalizeFn = (terminalContext = {}) => finalizeStream(terminalContext);
  transformStream.finalize = finalizeFn;
  return transformStream;
}

/** Resolve finalize callback from a transform stream (explicit fn preferred). */
export function resolveStreamFinalize(transformStream, explicitFn = null) {
  if (typeof explicitFn === "function") return explicitFn;
  if (typeof transformStream?.finalize === "function") {
    return (ctx) => transformStream.finalize(ctx);
  }
  return null;
}

export function createSSETransformStreamWithLogger(targetFormat, sourceFormat, provider = null, reqLogger = null, toolNameMap = null, model = null, connectionId = null, body = null, onStreamComplete = null, apiKey = null, customToolNames = null, requestTiming = null, usageEstimateBody = null) {
...
export function createPassthroughStreamWithLogger(provider = null, reqLogger = null, model = null, connectionId = null, body = null, onStreamComplete = null, apiKey = null, requestTiming = null, usageEstimateBody = null) {
  return createSSEStream({
