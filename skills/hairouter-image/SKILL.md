---
name: hairouter-image
description: Generate images via HAI-Router /v1/images/generations using OpenAI / Gemini Imagen / DALL-E / FLUX / MiniMax / SDWebUI / ComfyUI / Codex models. Use when the user wants to create, generate, draw, or render an image, picture, or text-to-image (txt2img).
---

# HAI-Router — Image Generation
...
```bash
curl $HAI_ROUTER_URL/v1/models/image | jq '.data[].id'
# Per-model params/options (size enum, quality enum, capabilities like edit)
curl "$HAI_ROUTER_URL/v1/models/info?id=openai/dall-e-3"
```

## Endpoint

`POST $HAI_ROUTER_URL/v1/images/generations`
...
```bash
curl -X POST "$HAI_ROUTER_URL/v1/images/generations?response_format=binary" \
...
```js
const r = await fetch(`${process.env.HAI_ROUTER_URL}/v1/images/generations`, {
  method: "POST",
