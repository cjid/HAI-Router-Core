...
const ROOT_CA_CN = "HAI-Router MITM Root CA";
const LEGACY_ROOT_CA_CN = "9Router MITM Root CA";
const ROOT_CA_EXPORT_BASENAME = "hairouter-root-ca.crt";
const LEGACY_ROOT_CA_EXPORT_BASENAME = "9router-root-ca.crt";
...
async function installCertMac(sudoPassword, certPath) {
  ...
  const deleteOld = `security delete-certificate -c "${ROOT_CA_CN}" /Library/Keychains/System.keychain 2>/dev/null || security delete-certificate -c "${LEGACY_ROOT_CA_CN}" /Library/Keychains/System.keychain 2>/dev/null || true`;
...
function checkCertInstalledLinux() {
  ...
  const certFile = `${config.dir}/${ROOT_CA_EXPORT_BASENAME}`;
...
async function installCertLinux(sudoPassword, certPath) {
  ...
  const config = getLinuxCertConfig();
  const destFile = `${config.dir}/${ROOT_CA_EXPORT_BASENAME}`;
