...
import { buildProxyOptionsFromCredentials } from "../../src/lib/network/connectionProxy.js";
import { getRuntimeGlobalStore } from "../shared/runtimeGlobals.js";
...
function getRefreshLocksMap() {
  return getRuntimeGlobalStore("refreshLocks", new Map());
}
...
export async function withCredentialRefreshLock(provider, credentials, refreshFn) {
  const refreshLocks = getRefreshLocksMap();
...
export async function refreshProviderCredentials(provider, credentials, log, proxyOptions = null) {
  ...
  const resolvedProxy = proxyOptions || buildProxyOptionsFromCredentials(credentials);

  return withCredentialRefreshLock(provider, credentials, async () => {
    const refreshed = await refreshTokenByProvider(provider, credentials, log, resolvedProxy);
