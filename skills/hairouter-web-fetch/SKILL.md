---
name: hairouter-web-fetch
description: Fetch URL → markdown / text / HTML via HAI-Router /v1/web/fetch using Firecrawl / Jina Reader / Tavily Extract / Exa Contents. Use when the user wants to scrape a webpage, extract URL content, read article, or convert a URL to markdown.
---

# HAI-Router — Web Fetch
...
```bash
curl $HAI_ROUTER_URL/v1/models/web | jq '.data[] | select(.kind=="webFetch") | .id'
# Per-provider params
curl "$HAI_ROUTER_URL/v1/models/info?id=firecrawl/fetch"
```
...
`POST $HAI_ROUTER_URL/v1/web/fetch`
...
### Jina Reader
```bash
curl -X POST $HAI_ROUTER_URL/v1/web/fetch \
  -H "Authorization: Bearer $HAI_ROUTER_KEY" \
  -H "Content-Type: application/json" \
  -d '{"model":"jina-reader","url":"https://hairouter.com","format":"markdown"}'
```
...
```js
const r = await fetch(`${process.env.HAI_ROUTER_URL}/v1/web/fetch`, {
  method: "POST",
