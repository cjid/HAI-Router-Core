...
function http2PostProto(url, headers, body, signal, timeoutMs, proxyOptions = null) {
  return (async () => {
    const skipGoForVitest = process.env.VITEST === "true" && process.env.HAI_GO_ENGINE_INTEGRATION !== "1";
    const { isGoEngineEnabled } = await import("../../src/lib/goEngine/workerManager.js");
    if (!skipGoForVitest && isGoEngineEnabled()) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      let client;
      try {
        client = await connectHttp2Client(urlObj.host, proxyOptions);
      } catch (error) {
        reject(error);
        return;
      }
...
async function fetchCursorCatalogViaFetch(url, headers, signal, proxyOptions) {
...
async function fetchCursorCatalog(credentials, signal, proxyOptions = null) {
  ...
  const response = isProxyConfigured(proxyOptions) && proxyOptions?.vercelRelayUrl && !proxyOptions?.connectionProxyUrl
    ? await fetchCursorCatalogViaFetch(url, headers, signal, proxyOptions)
    : await http2PostProto(url, headers, new Uint8Array(), signal, FETCH_TIMEOUT_MS, proxyOptions);
