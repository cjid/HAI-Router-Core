...
import { computeConnectTimeoutMs } from "../latency/adaptiveTimeout.js";
...
export class BaseExecutor {
  ...
  async execute({ model, body, stream, credentials, signal, log, proxyOptions = null, providerId = null, connectionId = null, requestTiming = null }) {
    ...
    for (let urlIndex = 0; urlIndex < fallbackCount; urlIndex++) {
      ...
      const adaptiveTimeout = (providerId && connectionId)
        ? computeConnectTimeoutMs(providerId, connectionId)
        : null;
      const timeoutMs = this.config?.timeoutMs || adaptiveTimeout || FETCH_CONNECT_TIMEOUT_MS;
      ...
      try {
        ...
        const headerTtfbMs = Date.now() - fetchT0;
        if (!stream) requestTiming?.markUpstreamFirstByte?.(fetchT0 + headerTtfbMs);
        const ct = response.headers?.get?.("content-type") || "";
        const cl = response.headers?.get?.("content-length") || "?";
        dbg("FETCH", `${this.provider.toUpperCase()} ← ${response.status} | ttft=${headerTtfbMs}ms | ct=${ct} | cl=${cl}`);
        ...
        return { response, url, headers, transformedBody, fetchTtfbMs: headerTtfbMs };
      } catch (error) {
        clearTimeout(connectTimer);
        lastError = error;
