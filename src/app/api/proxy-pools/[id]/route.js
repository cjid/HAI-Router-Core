...
import { invalidateEgressByPool } from "@/lib/network/connectionProxy";
...
function shouldInvalidateHealthTest(existing, updates) {
  if (updates.proxyUrl && updates.proxyUrl !== existing.proxyUrl) return true;
  if (updates.type && updates.type !== existing.type) return true;
  return false;
}
...
export async function PUT(request, { params }) {
  try {
    ...
    if (shouldInvalidateHealthTest(existing, normalized.updates)) {
      normalized.updates.testStatus = "unknown";
      normalized.updates.lastTestOk = null;
      normalized.updates.lastLatencyMs = null;
      normalized.updates.lastTestStatusCode = null;
      normalized.updates.lastError = null;
    }
