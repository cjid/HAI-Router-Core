...
function ProviderNode({ data }) {
  const { label, color, imageUrl, textIcon, active, completionBaseUrl, compatibility, apiType } = data;
  const [imgError, setImgError] = useState(false);
  const useDomainIcon = completionBaseUrl && compatibility;
  ...
  return (
    ...
    >
      ...
      {/* Provider icon */}
      <div
        className="w-8 h-8 rounded-md flex items-center justify-center shrink-0 border border-border/40 bg-bg-subtle/50 p-0.5"
        style={{ backgroundColor: useDomainIcon ? undefined : `${color}15` }}
      >
        {useDomainIcon ? (
          <CustomProviderIcon
...
function RouterNode({ data }) {
  ...
  const label = data.label || APP_CONFIG.productName;
  return (
    <div
    ...
    >
      ...
      <img
        ...
        alt={label}
        className={`w-6 h-6 mr-2 rounded-[6px] object-contain ${powering ? "topology-router-icon" : ""}`}
        loading="lazy"
        decoding="async"
      />
...
function buildLayout(providers, activeSet, lastSet, errorSet) {
  ...
  if (count === 0) {
    return {
      nodes: [{ id: "router", type: "router", position: { x: 0, y: 0 }, data: { activeCount: 0, label: APP_CONFIG.productName }, draggable: false }],
      edges: [],
    };
  }
  ...
  nodes.push({
    ...
    data: { activeCount: activeSet.size, label: APP_CONFIG.productName },
    draggable: false,
  });
  ...
  providers.forEach((p, i) => {
    ...
    const compatibility = p.compatibility || resolveCustomProviderCompatibility(p.provider);
    const data = {
      label: (config.name !== p.provider ? config.name : null) || p.nodeName || p.name || p.provider,
      color: config.color || (compatibility === "anthropic" ? "#D97757" : compatibility === "openai" ? "#10A37F" : "#6b7280"),
      imageUrl: p.baseUrl ? null : getProviderImageUrl(p.provider),
      completionBaseUrl: p.baseUrl || null,
      compatibility: p.baseUrl ? compatibility : null,
      apiType: p.apiType || null,
      textIcon: config.textIcon || (p.nodeName || p.provider || "?").slice(0, 2).toUpperCase(),
      active,
    };
...
export default function ProviderTopology({ providers = [], activeRequests = [], lastProvider = "", errorProvider = "" }) {
  ...
  return (
    <div ref={containerRef} className="h-[480px] w-full min-w-0 rounded-lg border border-border bg-bg-subtle/30">
      {providers.length === 0 ? (
