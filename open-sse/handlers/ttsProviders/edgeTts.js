...
async function getToken(credentials = null) {
  ...
  const res = await modalityFetch("https://www.bing.com/translator", {
...
async function ttsRequest(text, voiceId, token, credentials = null) {
  ...
  return modalityFetch("https://www.bing.com/tfettts?isVertical=1&&IG=1&IID=translator.5023&SFX=1", {
    method: "POST",
    body: body.toString(),
    headers: {
...
export default {
  ...
  async synthesize(text, model, credentials) {
    ...
    let res = await ttsRequest(text, voiceId, token, credentials);
    ...
    if (res.status === 429 || res.status === 403) {
      ...
      res = await ttsRequest(text, voiceId, token, credentials);
    }
