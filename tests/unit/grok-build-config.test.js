...
describe("grokBuildConfig", () => {
  it("creates independent main and per-type subagent model slots", () => {
    ...
    expect(parsed.default).toBe("hairouter");
    ...
    expect(parsed.subagentMappings).toMatchObject({
      "general-purpose": "hairouter-general-purpose",
      explore: "hairouter-explore",
      plan: "grok-4.5",
    });
  ...
  it("is idempotent and updates owned slots without duplicate sections", () => {
    ...
    expect(result.match(/^\[model\.hairouter\]$/gm)).toHaveLength(1);
    expect(result.match(/^\[model\.hairouter-general-purpose\]$/gm)).toHaveLength(1);
    expect(result.match(/^\[model\.hairouter-explore\]$/gm)).toHaveLength(1);
    expect(result.match(/^# hairouter-prev-subagent-explore/gm)).toHaveLength(1);
  ...
  it("blank override restores previous subagent mapping and removes owned slot", () => {
    ...
    expect(result).not.toContain("[model.hairouter-explore]");
    expect(parsed.subagentMappings["general-purpose"]).toBe("hairouter-general-purpose");
  });
  ...
  it("reset restores previous default and all previous subagent mappings", () => {
    ...
    expect(reset).not.toContain("[model.hairouter-");
    expect(reset).not.toContain("hairouter-prev-");
  ...
  it("removes mappings that were originally unset", () => {
    ...
    expect(parseGrokBuildConfig(applied).subagentMappings.plan).toBe("hairouter-plan");
  ...
  it("legacy callers without subagentModels leave existing overrides untouched", () => {
    ...
    expect(parsed.subagentMappings.explore).toBe("hairouter-explore");
  ...
  it("returns stable slot names only for supported subagent types", () => {
    expect(getGrokSubagentSlot("general-purpose")).toBe("hairouter-general-purpose");
    expect(getGrokSubagentSlot("explore")).toBe("hairouter-explore");
    expect(getGrokSubagentSlot("plan")).toBe("hairouter-plan");
