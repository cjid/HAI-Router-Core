...
export default function DefaultToolCard({ toolId, tool, isExpanded, onToggle, baseUrl, apiKeys, activeProviders = [], cloudEnabled = false, tunnelEnabled = false }) {
  ...
  const replaceVars = (text) => {
    const keyToUse = (selectedApiKey && selectedApiKey.trim()) 
      ? selectedApiKey 
      : (!cloudEnabled ? "sk_hairouter" : "your-api-key");
      ...
      <div className="mt-2 flex flex-col sm:flex-row sm:items-center gap-2">
        ...
        {modelValue && (
          <>
            ...
            >
              <MdiIcon name={copiedField === "model" ? "check" : "content_copy"} size={18} />
            </button>
            <button
        ...
        {tool.notes.map((note, index) => {
          ...
          return (
            <div key={index} className={`flex items-start gap-3 p-3 rounded-lg border ${bgClass}`}>
              <MdiIcon name={icon} size={18} className={iconClass} />
                ...
                <div className="mt-2 flex flex-col sm:flex-row sm:items-center gap-2">
                  ...
                  {item.copyable && (
                    <button
                      ...
                      className="shrink-0 px-3 py-2 bg-bg-secondary hover:bg-bg-tertiary rounded-lg border border-border transition-colors"
                    >
                      <MdiIcon name={copiedField === `${item.step}-${item.title}` ? "check" : "content_copy"} size={18} />
                    </button>
                  )}
                </div>
              )}
        ...
        {canShowGuide() && tool.codeBlock && (
          <div className="mt-2">
            <div className="flex items-center justify-between mb-2">
              ...
              >
                <MdiIcon name={copiedField === "codeblock" ? "check" : "content_copy"} size={14} />
  ...
  const renderIcon = () => {
    ...
    if (tool.icon) {
      return <MdiIcon name={tool.icon} size={20} style={{ color: tool.color }} />;
    }
