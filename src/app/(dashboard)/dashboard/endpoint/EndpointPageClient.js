...
import { Card, Button, Input, Modal, EndpointSkeleton, Toggle, ConfirmModal, SegmentedControl } from "@/shared/components";
...
function formatKeyUsageSummary(usage) {
  if (!usage || !usage.requests) {
    return "No usage in this period";
  }
  const cost = usage.cost > 0 ? ` · ~$${usage.cost.toFixed(2)}` : "";
  const cached = usage.cachedTokens > 0 ? ` · ${fmtCompact(usage.cachedTokens)} cached` : "";
  return `${fmtCompact(usage.requests)} req · ${fmtCompact(usage.promptTokens)} in · ${fmtCompact(usage.completionTokens)} out${cached}${cost}`;
}
...
export default function APIPageClient({ machineId }) {
 ...
 // Cloudflare Tunnel state
  ...
  const [keyUsageById, setKeyUsageById] = useState({});
  const [keyUsagePeriod, setKeyUsagePeriod] = useState("today");
  ...
  const loadSettings = async (signal) => {
    ...
    try {
      const [settingsRes, statusRes] = await Promise.all([
        ...
        fetch("/api/tunnel/status", { cache: "no-store", signal })
      ]);
      if (settingsRes.ok) {
  ...
  const fetchKeyUsage = useCallback(async (period, signal) => {
    setKeyUsageLoading(true);
    try {
      const res = await fetch(`/api/keys/usage?period=${encodeURIComponent(period)}`, { signal });
      if (!res.ok) return;
      const data = await res.json();
      const map = {};
      for (const row of data.keys || []) map[row.id] = row;
      setKeyUsageById(map);
    } catch (error) {
      if (isAbortError(error)) return;
  ...
  useEffect(() => {
    fetchKeyUsage(keyUsagePeriod, navSignal);
  }, [keyUsagePeriod, fetchKeyUsage, navSignal, keys.length]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch("/api/network/lan", { cache: "no-store" });
        if (!res.ok || cancelled) return;
        const data = await res.json();
        const ip = data.primary || data.addresses?.[0]?.address;
        if (!ip) {
          setLanUrl("");
          return;
        }
        const port = window.location.port || String(data.port || 20128);
        setLanUrl(`http://${ip}:${port}/v1`);
      } catch {
  ...
  return (
    <div className="flex flex-col gap-8">
      <UsageOverviewCard signal={navSignal} apiKeys={keys} />

      {/* Endpoint Card */}
      <Card>
        <h2 className="text-lg font-semibold mb-4 flex items-center gap-2">
          <MdiIcon name="api" size={18} className="text-primary" />
          API Endpoint
        </h2>
        ...
        <div className="flex flex-col gap-2">
          ...
          {lanUrl === null ? (
            <div className="flex items-stretch gap-2">
              <EndpointLabel>LAN</EndpointLabel>
              <div className="flex-1 flex items-center gap-2 px-3 py-2.5 rounded-[10px] border border-border bg-surface-2 text-sm text-text-muted min-h-[2.5rem]">
                <MdiIcon name="progress_activity" size={14} spin className="animate-spin" />
                Detecting…
              </div>
            </div>
          ) : lanUrl ? (
            ...
            <div className="flex items-stretch gap-2">
              <EndpointLabel>LAN</EndpointLabel>
              <div className="flex-1 flex items-center px-3 py-2.5 rounded-[10px] border border-border bg-surface-2 text-sm text-text-muted font-mono min-h-[2.5rem]">
          ...
          {/* Cloudflare Tunnel */}
          <div className="flex items-stretch gap-2">
            <EndpointLabel active={tunnelEnabled}>Tunnel</EndpointLabel>
            {tunnelEnabled && !tunnelLoading && tunnelReachable ? (
              <>
                <Input value={`${tunnelPublicUrl || tunnelUrl}/v1`} readOnly className="flex-1 self-stretch font-mono text-sm justify-center [&>div.relative]:h-full" inputClassName="h-full min-h-[2.5rem]" />
                <button
                  type="button"
                ...
                >
                  <MdiIcon name={copied === "tunnel_url" ? "check" : "content_copy"} size={18} />
                </button>
                <button
                  type="button"
                  onClick={() => setShowDisableTunnelModal(true)}
                  className="self-center p-2 hover:bg-red-500/10 rounded text-red-500 transition-colors shrink-0"
                  title="Disable Tunnel"
                >
                  <MdiIcon name="power_settings_new" size={18} />
                </button>
              </>
            ) : tunnelEnabled && !tunnelLoading && !tunnelReachable ? (
              <>
                <div className="flex-1 flex items-center gap-2 px-3 py-2.5 rounded-[10px] border border-amber-300 dark:border-amber-800 bg-amber-500/5 text-sm text-amber-600 dark:text-amber-400 min-h-[2.5rem]">
                  <MdiIcon name="progress_activity" size={14} spin className="animate-spin" />
              ...
              <>
                <div className="flex-1 flex items-center gap-2 px-3 py-2.5 rounded-[10px] border border-red-300 dark:border-red-800 bg-red-500/5 text-sm text-red-600 dark:text-red-400 min-h-[2.5rem]">
                  <MdiIcon name="error" size={14} />
                  {tunnelStatus.message}
                </div>
                <Button size="sm" icon="cloud_upload" className="self-center" onClick={() => setShowEnableTunnelModal(true)}>Enable</Button>
              </>
            ) : tunnelChecking ? (
              <>
                <div className="flex-1 flex items-center gap-2 px-3 py-2.5 rounded-[10px] border border-border bg-surface-2 text-sm text-text-muted min-h-[2.5rem]">
              ...
              <Button
                size="sm"
                icon="cloud_upload"
                className="self-center min-h-[2.5rem]"
                onClick={() => {
                  if (isLoginUnsafe) {
          ...
          {/* Tailscale */}
          <div className="flex items-stretch gap-2">
            <EndpointLabel active={tsEnabled}>Tailscale</EndpointLabel>
            {tsEnabled && !tsLoading && tsReachable ? (
              <>
                <Input value={`${tsUrl}/v1`} readOnly className="flex-1 self-stretch font-mono text-sm justify-center [&>div.relative]:h-full" inputClassName="h-full min-h-[2.5rem]" />
                <button
                  type="button"
                  onClick={() => copy(`${tsUrl}/v1`, "ts_url")}
                  className="self-center p-2 hover:bg-black/5 dark:hover:bg-white/5 rounded text-text-muted hover:text-primary transition-colors shrink-0"
                >
                  <MdiIcon name={copied === "ts_url" ? "check" : "content_copy"} size={18} />
                </button>
                <button
                  type="button"
              ...
              <>
                ...
                <Button size="sm" icon="vpn_lock" className="self-center min-h-[2.5rem]" onClick={handleOpenTsModal}>Enable</Button>
              </>
            ) : (
              <Button
                size="sm"
                icon="vpn_lock"
                className="self-center min-h-[2.5rem] bg-linear-to-r from-indigo-500 to-purple-500 hover:from-indigo-600 hover:to-purple-600 text-white!"
                onClick={() => {
                  if (isLoginUnsafe) {
      ...
      <Card id="require-api-key">
        <div className="flex flex-col gap-3 mb-4 sm:flex-row sm:items-center sm:justify-between">
          <h2 className="text-lg font-semibold flex items-center gap-2">
            <MdiIcon name="vpn_key" size={18} className="text-primary" />
            API Keys
          </h2>
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
            <SegmentedControl
              options={KEY_USAGE_PERIODS}
              value={keyUsagePeriod}
              onChange={setKeyUsagePeriod}
              size="sm"
              className="w-full sm:w-auto"
            />
            <Button icon="add" onClick={() => setShowAddModal(true)} className="w-full sm:w-auto">
              Create Key
            </Button>
          </div>
        </div>
            ...
            {keys.map((key) => (
              ...
              >
                <div className="flex-1 min-w-0">
                  ...
                  <p className={`text-xs mt-1 font-mono ${keyUsageById[key.id]?.requests ? "text-primary" : "text-text-muted"}`}>
                    {keyUsageLoading && !keyUsageById[key.id]
                      ? "Loading usage…"
                      : formatKeyUsageSummary(keyUsageById[key.id])}
                  </p>
                  {key.isActive === false && (
        ...
        <div className="flex flex-col gap-4">
          <div className="bg-surface-2 border border-border-subtle rounded-lg p-4">
            <div className="flex items-start gap-3">
              <MdiIcon name="cloud_upload" size={18} className="text-primary" />
              <div>
                <p className="text-sm text-text-main font-medium mb-1">
                  Cloudflare Tunnel
                </p>
                <p className="text-sm text-text-muted">
                  Expose your local HAI-Router to the internet. No port forwarding, no static IP needed. Share endpoint URL with your team or use it in Cursor, Cline, and other AI tools from anywhere.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            {TUNNEL_BENEFITS.map((benefit) => (
              <div key={benefit.title} className="flex flex-col items-center text-center p-3 rounded-lg bg-sidebar/50">
                <MdiIcon name={benefit.icon} size={20} className="text-primary mb-1" />
