...
const hasHairouterConfig = (auth) => {
  if (!auth) return false;
  const entry = auth["openai-compatible"] || auth["hairouter"] || auth["9router"];
  if (!entry) return false;
  const baseUrl = entry.baseUrl || entry.baseURL || "";
  return baseUrl.includes("localhost") || baseUrl.includes("127.0.0.1") || baseUrl.includes("hairouter") || baseUrl.includes("9router");
};
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasHairouter: hasHairouterConfig(auth),
...
export async function POST(request) {
  try {
    ...
    try {
      ...
      vscode["kilocode.customProvider"] = { name: "HAI-Router", baseURL: normalizedBaseUrl, apiKey };
...
export async function DELETE() {
  try {
    ...
    return NextResponse.json({ success: true, message: "HAI-Router settings removed from Kilo Code" });
  } catch (error) {
