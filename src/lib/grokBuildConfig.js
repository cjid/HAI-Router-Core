...
/** Canonical Grok Build TOML model slot prefix. */
export const GROK_MAIN_MODEL_SLOT = "hairouter";

/** Legacy Grok Build slot — read-only migration. */
...
const UNSET_SENTINEL = "__hairouter_unset__";
...
const previousDefaultRegExp = /^# (?:hairouter|9router)-prev-default = "([^"]*)"[ \t]*\r?\n?/m;
const previousSubagentRegExp = (type) =>
  new RegExp(
    `^# (?:hairouter|9router)-prev-subagent-${escapeRegExp(type)} = "([^"]*)"[ \\t]*\\r?\\n?`,
    "m",
  );

function resolveMainSlot(toml) {
  if (sectionRegExp(`model.${GROK_MAIN_MODEL_SLOT}`).test(toml)) return GROK_MAIN_MODEL_SLOT;
  if (sectionRegExp(`model.${LEGACY_GROK_MAIN_MODEL_SLOT}`).test(toml)) return LEGACY_GROK_MAIN_MODEL_SLOT;
  return GROK_MAIN_MODEL_SLOT;
}
...
function buildModelSection({ slot, model, baseUrl, apiKey, contextWindow, name }) {
  const lines = [
    ...
    `description = ${tomlString(`Routed via ${PRODUCT.displayName} gateway`)}`,
...
function insertMarker(toml, marker) {
  ...
  const legacySection = sectionRegExp(`model.${LEGACY_GROK_MAIN_MODEL_SLOT}`);
  if (mainSection.test(toml)) {
    return toml.replace(mainSection, (section) => `${marker}${section}`);
  }
  if (legacySection.test(toml)) {
    return toml.replace(legacySection, (section) => `${marker}${section}`);
  }
  const prefix = toml.length > 0 && !toml.endsWith("\n") ? `${toml}\n` : toml;
...
function rememberPreviousDefault(toml) {
  ...
  const activeSlot = resolveMainSlot(toml);
  if (!current || current === activeSlot || current === GROK_MAIN_MODEL_SLOT || current === LEGACY_GROK_MAIN_MODEL_SLOT) {
    return toml;
  }
  return insertMarker(toml, `# hairouter-prev-default = ${tomlString(current)}\n`);
}
...
function rememberPreviousSubagent(toml, type) {
  ...
  return insertMarker(
    toml,
    `# hairouter-prev-subagent-${type} = ${tomlString(previous)}\n`,
  );
}

function restorePreviousSubagent(toml, type) {
  const regexp = previousSubagentRegExp(type);
  const rawPrevious = toml.match(regexp)?.[1] || UNSET_SENTINEL;
  const previous = rawPrevious === LEGACY_UNSET_SENTINEL ? UNSET_SENTINEL : rawPrevious;
  let next = toml.replace(regexp, "");
  const slot = modelSlot(type);
  const legacySlot = `${LEGACY_GROK_MAIN_MODEL_SLOT}-${type}`;
  const current = getSectionField(next, SUBAGENT_MODELS_SECTION, type);
  if (current !== slot && current !== legacySlot) {
    return next;
  }
...
export function parseGrokBuildConfig(toml) {
  const mainSlot = resolveMainSlot(toml);
  ...
  for (const type of GROK_SUBAGENT_TYPES) {
    ...
    const canonicalSlot = modelSlot(type);
    const legacySlot = `${LEGACY_GROK_MAIN_MODEL_SLOT}-${type}`;
    subagentModels[type] = mapping === canonicalSlot || mapping === legacySlot
      ? parseModelSection(toml, mapping)
      : null;
  }

  return {
    model: parseModelSection(toml, mainSlot),
  ...
  if (subagentModels && typeof subagentModels === "object") {
    for (const type of GROK_SUBAGENT_TYPES) {
      ...
      if (selected?.model) {
        ...
        next = removeModelSection(next, `${LEGACY_GROK_MAIN_MODEL_SLOT}-${type}`);
        next = upsertModelSection(next, {
          slot,
