...
async function probeWebProvider(provider, apiKey) {
  ...
  const res = await providerFetch(url, { method: cfg.method, headers, body, signal: AbortSignal.timeout(8000) });
  ...
  try {
    ...
    try {
      if (isOpenAICompatibleProvider(provider)) {
        ...
        const res = await providerFetch(modelsUrl, {
      ...
      if (isCustomEmbeddingProvider(provider)) {
        ...
        const modelsRes = await providerFetch(`${baseUrl}/models`, {
          headers: { "Authorization": `Bearer ${apiKey}` },
        });
        if (modelsRes.ok) {
        ...
        const embedRes = await providerFetch(`${baseUrl}/embeddings`, {
          method: "POST",
      ...
      if (isAnthropicCompatibleProvider(provider)) {
        ...
        const res = await providerFetch(messagesUrl, {
          method: "POST",
          headers: {
            "x-api-key": apiKey,
      ...
      if (provider === "cloudflare-ai") {
        ...
        const cfRes = await providerFetch(url, {
          method: "POST",
          headers: { "Authorization": `Bearer ${apiKey}`, "Content-Type": "application/json" },
          body: JSON.stringify({
      ...
      if (provider === "azure") {
        ...
        const azureRes = await providerFetch(url, {
          method: "POST",
          headers,
          body: JSON.stringify({
            messages: [{ role: "user", content: "test" }],
      ...
      switch (provider) {
        case "openai":
          const openaiRes = await providerFetch("https://api.openai.com/v1/models", {
            headers: { "Authorization": `Bearer ${apiKey}` },
          });
        ...
        case "vercel-ai-gateway":
          const vercelAiGatewayRes = await providerFetch("https://ai-gateway.vercel.sh/v1/models", {
            headers: { "Authorization": `Bearer ${apiKey}` },
          });
        ...
        case "anthropic":
          const anthropicRes = await providerFetch("https://api.anthropic.com/v1/messages", {
            method: "POST",
            headers: {
              "x-api-key": apiKey,
              "anthropic-version": "2023-06-01",
        ...
        case "gemini":
          const geminiRes = await providerFetch(`https://generativelanguage.googleapis.com/v1/models?key=${apiKey}`);
          isValid = geminiRes.ok;
          break;

        case "openrouter":
          const openrouterRes = await providerFetch("https://openrouter.ai/api/v1/models", {
            headers: { "Authorization": `Bearer ${apiKey}` },
          });
        ...
        case "volcengine-ark":
        case "byteplus": {
          const res = await providerFetch(PROVIDERS[provider]?.baseUrl, {
            method: "POST",
            headers: {
              "Authorization": `Bearer ${apiKey}`,
              "content-type": "application/json",
        ...
        case "nvidia": {
          ...
          const res = await providerFetch(endpoints[provider], { headers, signal: AbortSignal.timeout(8000) });
        ...
        case "opencode-go": {
          const res = await providerFetch("https://opencode.ai/zen/go/v1/chat/completions", {
            method: "POST",
        ...
        case "deepgram": {
          const res = await providerFetch("https://api.deepgram.com/v1/projects", {
        ...
        case "blackbox": {
          const res = await providerFetch("https://api.blackbox.ai/chat/completions", {
            method: "POST",
            headers: {
              "Authorization": `Bearer ${apiKey}`,
              "Content-Type": "application/json",
        ...
        case "grok-web": {
          ...
          const res = await providerFetch("https://grok.com/rest/app-chat/conversations/new", {
            method: "POST",
            headers: {
              Accept: "*/*",
        ...
        case "perplexity-web": {
          ...
          const res = await providerFetch("https://www.perplexity.ai/rest/sse/perplexity_ask", {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
        ...
        default: {
          ...
          let probeOk = null;
          try {
            const probeRes = await providerFetch(modelsUrl, { headers, signal: AbortSignal.timeout(8000) });
          ...
          const chatRes = await providerFetch(cfg.baseUrl, {
            method: "POST",
            headers,
