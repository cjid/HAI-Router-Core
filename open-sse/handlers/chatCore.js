...
import { recordUpstreamRateLimit } from "../concurrency/index.js";
import { waitForProviderPacing } from "../concurrency/providerPacing.js";
import { createRequestLogger } from "../utils/requestLogger.js";
import { getModelTargetFormat, getModelSupportedFormats, getModelStrip, getModelUpstreamId, getModelType, PROVIDER_ID_TO_ALIAS } from "../config/providerModels.js";
import { PROVIDERS } from "../config/providers.js";
import { createErrorResult, parseUpstreamError, formatProviderError, createTransportErrorResult, createUpstreamErrorResult } from "../utils/error.js";
import { HTTP_STATUS, TOKEN_SAVER_HEADER, LEGACY_TOKEN_SAVER_HEADER } from "../config/runtimeConfig.js";
import { handleBypassRequest } from "../utils/bypassHandler.js";
import { trackPendingRequest, appendRequestLog, saveRequestDetail } from "@/lib/usageDb.js";
import { buildProxyOptionsFromCredentials, cacheConnectionEgress, logEgressAssignment, enrichProxyOptions } from "@/lib/network/connectionProxy.js";
...
export async function handleChatCore({ body, modelInfo, credentials, log, onCredentialsRefreshed, onRequestSuccess, onDisconnect, onAdmissionRelease, clientRawRequest, connectionId, userAgent, apiKey, ccFilterNaming, rtkEnabled, headroomEnabled, headroomUrl, headroomCompressUserMessages, headroomTimeoutMs, cavemanEnabled, cavemanLevel, ponytailEnabled, ponytailLevel, pxpipeEnabled, pxpipeMinChars, pxpipeTimeoutMs, pxpipeTransform, onPxpipeEvent, sourceFormatOverride, providerThinking, signal = null, streamState = null, requestTiming = null, requestId = null, clientFormat = null }) {
  ...
  const requestStartTime = requestTiming?.requestStartMs ?? Date.now();
  requestTiming?.mark?.("request_start", requestStartTime);
  ...
  const errorCtx = {
    ...
    connectionId,
    model,
    clientFormat: clientFormat || sourceFormat,
    log,
    reqTag,
  };
  ...
  if (passthrough) {
    ...
    if (!translatedBody) {
      ...
      releaseAdmissionOnce();
      return createErrorResult(HTTP_STATUS.BAD_REQUEST, `Failed to translate request for ${sourceFormat} → ${targetFormat}`, null, { ...errorCtx, phase: "translate" });
    }
  ...
  const hdrs = clientRawRequest?.headers || {};
  const tokenSaverOff =
    hdrs[TOKEN_SAVER_HEADER]?.toLowerCase() === "off"
    || hdrs[LEGACY_TOKEN_SAVER_HEADER]?.toLowerCase() === "off";
  const tokenSaverEnabled = !tokenSaverOff;
  ...
  let pendingToken = trackPendingRequest(model, provider, connectionId, true);
  appendRequestLog({ model, provider, connectionId, status: "PENDING" }).catch(() => { });

  const finishPending = (isError = false) => {
    if (!pendingToken) return;
    trackPendingRequest(model, provider, connectionId, false, isError, pendingToken);
    pendingToken = null;
  };

  const finishRequest = (isError = false) => {
    finishPending(isError);
    releaseAdmissionOnce();
  };
  ...
  const proxyOptions = enrichProxyOptions(buildProxyOptionsFromCredentials(credentials), {
    providerId: provider,
    connectionId,
    sessionId: sessionSeed,
  });
  if (connectionId) cacheConnectionEgress(connectionId, proxyOptions);
  logEgressAssignment(log, {
    connectionId,
    provider,
    proxyOptions,
    affinity: "existing",
  ...
  requestTiming?.markUpstreamDispatch?.();
  try {
    const pacingStart = Date.now();
    await waitForProviderPacing(provider, { signal: streamController.signal });
    const pacingWaitMs = Date.now() - pacingStart;
    if (pacingWaitMs > 50) {
      log?.debug?.("PACING", `${provider.toUpperCase()} | human-gap wait ${pacingWaitMs}ms before upstream`);
    }
    ...
    if (error.name === "AbortError") {
      ...
      releaseAdmissionOnce();
      return createErrorResult(499, "Request aborted", null, { ...errorCtx, phase: "upstream" });
    }
    ...
    releaseAdmissionOnce();
    return createTransportErrorResult(error, { ...errorCtx, phase: "upstream" });
  }

  // Handle 401/403 - try token refresh (skip for noAuth providers; skip after stream started)
  if (!executor.noAuth && !streamState?.started && (providerResponse.status === HTTP_STATUS.UNAUTHORIZED || providerResponse.status === HTTP_STATUS.FORBIDDEN)) {
    try {
      const newCredentials = await withCredentialRefreshLock(provider, credentials, async () => refreshWithRetry(async () => {
  ...
  if (!providerResponse.ok) {
    ...
    if (providerResponse.status === 429) {
      recordUpstreamRateLimit(provider, connectionId, providerResponse);
    }
    const parsed = await parseUpstreamError(providerResponse, executor);
    appendRequestLog({ model, provider, connectionId, status: `FAILED ${parsed.statusCode}` }).catch(() => { });
    saveRequestDetail(buildRequestDetail({
      provider, model, connectionId,
      latency: { ttft: 0, total: Date.now() - requestStartTime },
      tokens: { prompt_tokens: 0, completion_tokens: 0 },
      ...
      response: { error: parsed.message, status: parsed.statusCode, thinking: null },
      pxpipe: pxpipeSummary,
      status: "error"
    })).catch(() => { });

    const errMsg = formatProviderError(new Error(parsed.message), provider, model, parsed.statusCode);
    if (log?.errorLine) {
      const urlStr = providerUrl ? `\n    URL: ${providerUrl}` : "";
      log.errorLine(reqTag, "✗", `ERROR ${parsed.statusCode} · ${provider}/${model} · ${Date.now() - requestStartTime}ms${urlStr}\n    ${errMsg}`);
    }
    reqLogger.logError(new Error(parsed.message), finalBody || translatedBody);
    releaseAdmissionOnce();
    return {
      ...createUpstreamErrorResult(parsed, { ...errorCtx, phase: "upstream" }),
  ...
  const requestKind = (() => {
    ...
    const raw = headers["x-9r-model-test"] ?? headers["X-9r-Model-Test"];
    return raw === "1" || raw === "true" ? "model_test" : null;
  })();
  ...
  const sharedCtx = {
    ...
    reqTag, log, streamState, releaseAdmissionOnce, requestTiming, errorCtx, requestKind,
    proxyOptions,
  };
  ...
  if (!clientRequestedStreaming && providerRequiresStreaming) {
    ...
    if (result) { streamController.handleComplete(); releaseAdmissionOnce(); return { ...result, streamStarted: !!streamState?.started }; }
  }
  ...
  if (!stream) {
    ...
    releaseAdmissionOnce();
    return { ...result, streamStarted: !!streamState?.started };
  }
  ...
  const result = await handleStreamingResponse({
    ...sharedCtx,
    providerResponse, sourceFormat, targetFormat: providerResponseFormat,
    userAgent, reqLogger, toolNameMap, customToolNames, streamController,
    onStreamComplete, streamDetailId,
  });
  return { ...result, streamStarted: !!streamState?.started, latencyRecorded: !!result.latencyRecorded };
}
