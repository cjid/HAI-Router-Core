...
import { resolveConnectionProxyConfig, buildProxyOptions, buildProxyOptionsFromCredentials } from "@/lib/network/connectionProxy";
...
const OAUTH_TEST_CONFIG = {
  ...
  github: {
    ...
    authHeader: "Authorization",
    authPrefix: "Bearer ",
    extraHeaders: { "User-Agent": PRODUCT.userAgent, "Accept": "application/vnd.github+json" },
  },
  iflow: {
...
async function probeClineAccessToken(accessToken) {
  const res = await providerFetch("https://api.cline.bot/api/v1/users/me", {
    method: "GET",
...
async function refreshOAuthToken(connection) {
  ...
  try {
    ...
    if (provider === "claude") {
      const response = await providerFetch(CLAUDE_CONFIG.tokenUrl, {
        method: "POST",
    ...
    if (provider === "kiro") {
      ...
      const response = await providerFetch(KIRO_CONFIG.socialRefreshUrl, {
        method: "POST",
    ...
    if (provider === "cline") {
      const response = await providerFetch(CLINE_CONFIG.refreshUrl, {
        method: "POST",
        headers: { "Content-Type": "application/json", Accept: "application/json" },
        body: JSON.stringify({
          refreshToken,
...
async function fetchWithConnectionProxy(url, options = {}, effectiveProxy = null) {
  ...
  const proxyOpts = effectiveProxy
    ? buildProxyOptions(effectiveProxy)
    : { providerId: "connection-test" };
  return proxyAwareFetch(url, options, proxyOpts);
}
...
async function testApiKeyConnection(connection, effectiveProxy = null) {
  ...
  try {
    switch (connection.provider) {
      ...
      case "ollama": {
        const res = await providerFetch("https://ollama.com/api/tags", { headers: { Authorization: `Bearer ${connection.apiKey}` } }, { providerId: "ollama" });
        return { valid: res.ok, error: res.ok ? null : "Invalid API key" };
      }
      case "ollama-local": {
        const host = resolveOllamaLocalHost(connection);
        const res = await providerFetch(`${host}/api/tags`, {}, { providerId: connection.provider || "ollama" });
