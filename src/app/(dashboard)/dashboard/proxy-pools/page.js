...
import { Badge, Button, Card, ProxyPoolsSkeleton, Input, Modal, Toggle, ConfirmModal } from "@/shared/components";
import { useNotificationStore } from "@/store/notificationStore";
import { useDateTimeFormat } from "@/shared/hooks/useDateTimeFormat";
import { getProxyLatencySemantic, getSemanticTextClass } from "@/shared/utils/statusSemantic";

const TEST_FEEDBACK_MS = 3500;
...
function formatDateTimeLocal(value, formatFn) {
...
export default function ProxyPoolsPage() {
  ...
  const testFeedbackTimers = useRef({});
  ...
  const handleTest = async (proxyPoolId) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setTestFeedback((prev) => ({ ...prev, [proxyPoolId]: "error" }));
        return;
      }
      ...
      if (data.ok) {
        ...
        notify.success(latency ? `Proxy test successful · ${latency}` : "Proxy test successful");
        setTestFeedback((prev) => ({ ...prev, [proxyPoolId]: "success" }));
      } else {
        notify.error(data.errorMessage || data.error || "Proxy test failed");
        setTestFeedback((prev) => ({ ...prev, [proxyPoolId]: "error" }));
      }
    } catch (error) {
      ...
      setTestingId(null);
      if (testFeedbackTimers.current[proxyPoolId]) {
        clearTimeout(testFeedbackTimers.current[proxyPoolId]);
      }
      testFeedbackTimers.current[proxyPoolId] = setTimeout(() => {
        setTestFeedback((prev) => {
          if (!prev[proxyPoolId]) return prev;
          const next = { ...prev };
          delete next[proxyPoolId];
          return next;
        });
        delete testFeedbackTimers.current[proxyPoolId];
      }, TEST_FEEDBACK_MS);
    }
  };

  const resolveTestIcon = (poolId) => {
    if (testingId === poolId) return { name: "progress_activity", spin: true };
    if (testFeedback[poolId] === "success") return { name: "check_circle", spin: false };
    if (testFeedback[poolId] === "error") return { name: "error", spin: false };
    return { name: "science", spin: false };
  };

  const formatLastTested = (pool) => {
    const when = formatDateTimeLocal(pool.lastTestedAt, formatDateTime);
    if (!pool.lastTestedAt) return `Last tested: ${when}`;
    if (pool.lastLatencyMs != null && pool.testStatus === "active") {
      return `Last tested: ${when} · ${pool.lastLatencyMs} ms`;
    }
    if (pool.testStatus === "error") {
      return `Last tested: ${when} · Failed`;
    }
    if (pool.testStatus === "unknown") {
      return `Last tested: ${when} · Untested`;
    }
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        ...
        <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
          <div className="relative" ref={relayMenuRef}>
            ...
            >
              Deploy Relay
              <MdiIcon name={showRelayMenu ? "expand_less" : "expand_more"} size={18} className="ml-1" />
            </Button>
            ...
            {showRelayMenu && (
              <div className="absolute left-0 top-full z-50 mt-1 w-48 rounded-xl border border-black/10 bg-white p-1 shadow-xl dark:border-white/10 dark:bg-zinc-900 sm:left-auto sm:right-0">
                ...
                >
                  <MdiIcon name="cloud" size={20} className="text-orange-500" />
                  Cloudflare Relay
                </button>
                <button
                  onClick={() => {
                  ...
                  className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-sm text-text-main transition-colors hover:bg-black/5 dark:hover:bg-white/5"
                >
                  <MdiIcon name="cloud_upload" size={20} className="text-blue-500" />
                  Vercel Relay
                </button>
                <button
                  onClick={() => {
                  ...
                  className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-sm text-text-main transition-colors hover:bg-black/5 dark:hover:bg-white/5"
                >
                  <MdiIcon name="terminal" size={20} className="text-green-500" />
                  Deno Relay
                </button>
              </div>
            )}
      ...
      <Card>
        ...
        {(selectedIds.length > 0 || healthChecking) && (
          <div className="mb-4 flex flex-wrap items-center gap-2 rounded-lg border border-primary/30 bg-primary/5 px-3 py-2">
            <MdiIcon name="checklist" size={18} className="text-primary" />
        ...
        {proxyPools.length === 0 ? (
          ...
          <div className="flex flex-col divide-y divide-black/[0.04] dark:divide-white/[0.05]">
            {proxyPools.map((pool) => {
              const testIcon = resolveTestIcon(pool.id);
              return (
              <div key={pool.id} className="flex flex-col gap-3 py-3 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex items-start gap-3 min-w-0 flex-1">
                  ...
                  <p className="text-[11px] text-text-muted mt-1">
                    {formatLastTested(pool)}
                    {pool.lastError ? ` · ${pool.lastError}` : ""}
                  </p>
                  {pool.lastLatencyMs != null && pool.testStatus === "active" ? (
                    <p className={`text-[11px] font-medium mt-0.5 ${getSemanticTextClass(getProxyLatencySemantic(pool.lastLatencyMs))}`}>
                      Latency: {pool.lastLatencyMs} ms
                    </p>
                  ) : null}
                  </div>
                </div>
                ...
                <div className="flex items-center justify-end gap-1">
                  ...
                  <button
                    ...
                    aria-busy={testingId === pool.id || undefined}
                  >
                    <MdiIcon
                      name={testIcon.name}
                      size={18}
                      spin={testIcon.spin}
                      className={testingId === pool.id ? "animate-process-spin" : undefined}
                    />
                  </button>
                  <button
