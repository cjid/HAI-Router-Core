...
export default {
  ...
  async parseResponse(response, { headers, credentials }) {
    const { status_url, response_url } = await response.json();
    const deadline = Date.now() + POLL_TIMEOUT_MS;
    while (Date.now() < deadline) {
      await sleep(POLL_INTERVAL_MS);
      const r = await modalityFetch(status_url, { headers }, credentials);
      if (!r.ok) throw new Error(`Fal status ${r.status}`);
      const s = await r.json();
      if (s.status === "COMPLETED") {
        const fr = await modalityFetch(response_url, { headers }, credentials);
