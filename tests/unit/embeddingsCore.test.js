...
// Embeddings use modalityFetch (not global fetch)
vi.mock("../../open-sse/handlers/modalityProxy.js", () => ({
  modalityFetch: vi.fn(),
  resolveModalityProxyOptions: vi.fn(() => ({})),
}));

import { handleEmbeddingsCore } from "../../open-sse/handlers/embeddingsCore.js";
import { modalityFetch } from "../../open-sse/handlers/modalityProxy.js";
...
describe("buildEmbeddingsBody", () => {
  beforeEach(() => {
    vi.mocked(modalityFetch).mockReset();
  });

  it("single string input — includes model and input, default encoding_format=float", async () => {
    vi.mocked(modalityFetch).mockResolvedValueOnce(makeProviderResponse(VALID_EMBEDDING_RESPONSE));

    await handleEmbeddingsCore(makeOptions({
      body: { model: "text-embedding-ada-002", input: "Hello world" },
    }));

    const [, init] = vi.mocked(modalityFetch).mock.calls[0];
  ...
  it("gemini single input forwards dimensions as outputDimensionality", async () => {
    vi.mocked(modalityFetch).mockResolvedValueOnce(makeProviderResponse({
...
describe("buildEmbeddingsUrl", () => {
  ...
  it("openai → https://api.openai.com/v1/embeddings", async () => {
    ...
    const [url] = vi.mocked(modalityFetch).mock.calls[0];
  ...
  it("vercel-ai-gateway → https://ai-gateway.vercel.sh/v1/embeddings", async () => {
    ...
    const [url, init] = vi.mocked(modalityFetch).mock.calls[0];
    const sent = JSON.parse(init.body);
  ...
  it("unsupported provider (e.g. gemini-cli) → 400 error, no fetch called", async () => {
    ...
    expect(vi.mocked(modalityFetch)).not.toHaveBeenCalled();
    expect(result.success).toBe(false);
    expect(result.status).toBe(400);
    expect(result.internalError?.upstreamMessage).toMatch(/does not support embeddings/i);
  });
...
describe("handleEmbeddingsCore — input validation", () => {
  ...
  it("missing input → 400 Bad Request", async () => {
    ...
    expect(result.success).toBe(false);
    expect(result.status).toBe(400);
    expect(result.internalError?.upstreamMessage || result.error).toMatch(/missing required field: input/i);
  });
  ...
  it("input is a number → 400 Bad Request", async () => {
    ...
    expect(result.success).toBe(false);
    expect(result.status).toBe(400);
    expect(result.internalError?.upstreamMessage || result.error).toMatch(/input must be a string or array/i);
  });
  ...
  it("empty array input passes validation and reaches provider", async () => {
    vi.mocked(modalityFetch).mockResolvedValueOnce(
      makeProviderResponse(VALID_EMBEDDING_RESPONSE),
    );
    const result = await handleEmbeddingsCore(makeOptions({
      body: { model: "text-embedding-ada-002", input: [] },
    }));
    expect(vi.mocked(modalityFetch)).toHaveBeenCalledOnce();
...
describe("handleEmbeddingsCore — success path", () => {
  ...
  it("does not call onRequestSuccess on provider error", async () => {
    vi.mocked(modalityFetch).mockResolvedValueOnce(makeProviderErrorResponse(500, "Server exploded"));
    const onRequestSuccess = vi.fn();
  ...
  it("provider response with non-standard format is passed through as-is", async () => {
    ...
    vi.mocked(modalityFetch).mockResolvedValueOnce(makeProviderResponse(nonStandardBody));
...
describe("handleEmbeddingsCore — provider error handling", () => {
  ...
  it("provider 400 → returns success=false with status 400", async () => {
    vi.mocked(modalityFetch).mockResolvedValueOnce(makeProviderErrorResponse(400, "Bad model"));
  ...
  it("provider 429 → returns success=false with status 429", async () => {
    vi.mocked(modalityFetch).mockResolvedValueOnce(makeProviderErrorResponse(429, "Rate limit exceeded"));
  ...
  it("provider 500 → returns success=false with status 500", async () => {
    vi.mocked(modalityFetch).mockResolvedValueOnce(makeProviderErrorResponse(500, "Internal error"));
  ...
  it("network error (fetch throws) → returns 502 Bad Gateway", async () => {
    vi.mocked(modalityFetch).mockRejectedValueOnce(new Error("ECONNREFUSED"));

    const result = await handleEmbeddingsCore(makeOptions());

    expect(result.success).toBe(false);
    expect(result.status).toBe(502);
    expect(result.internalError?.upstreamMessage || result.error).toMatch(/ECONNREFUSED/);
  });
...
describe("handleEmbeddingsCore — token refresh on 401/403", () => {
  ...
  it("on 401, attempts retry after refresh; succeeds if refresh gives new token", async () => {
    ...
    expect(vi.mocked(modalityFetch).mock.calls.length).toBeGreaterThanOrEqual(1);
  });
