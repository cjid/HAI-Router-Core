...
export default function OAuthModal({ isOpen, provider, providerInfo, onSuccess, onClose, oauthMeta, idcConfig, proxyPools }) {
  ...
  const resolvedProxyPoolId = proxyPoolId === NONE_PROXY_POOL_VALUE ? null : proxyPoolId;

  const buildOAuthMeta = useCallback(() => {
    if (!resolvedProxyPoolId && !oauthMeta) return undefined;
    return {
      ...(oauthMeta || {}),
      ...(resolvedProxyPoolId ? { proxyPoolId: resolvedProxyPoolId } : {}),
    };
  }, [oauthMeta, resolvedProxyPoolId]);
  ...
  }, [authData, provider, onSuccess, buildOAuthMeta, resolvedProxyPoolId]);
  ...
  }, [authData, onSuccess, resolvedProxyPoolId]);
  ...
  const startPolling = useCallback(async (deviceCode, codeVerifier, interval, extraData, deadlineMs) => {
    ...
    setStep("error");
    setPolling(false);
  }, [provider, onSuccess, resolvedProxyPoolId]);

  const appendProxyPoolQuery = useCallback((url) => {
    if (resolvedProxyPoolId) url.searchParams.set("proxy_pool_id", resolvedProxyPoolId);
    return url;
  }, [resolvedProxyPoolId]);
  ...
  const startProxyFlow = useCallback(async (providerId) => {
    ...
    await fetch(`/api/oauth/${providerId}/register-session?state=${encodeURIComponent(authData.state)}`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  const startOAuthFlow = useCallback(async () => {
    ...
    try {
      ...
      if (deviceCodeProviders.includes(provider)) {
        ...
        const deviceCodeUrl = appendProxyPoolQuery(
          new URL(`/api/oauth/${provider}/device-code`, window.location.origin),
        );
      ...
      const authorizeUrl = appendProxyPoolQuery(new URL(`/api/oauth/${provider}/authorize`, window.location.origin));
      authorizeUrl.searchParams.set("redirect_uri", redirectUri);
      const mergedMeta = buildOAuthMeta();
      if (mergedMeta) {
        Object.entries(mergedMeta).forEach(([k, v]) => { if (v) authorizeUrl.searchParams.set(k, v); });
      } else if (oauthMeta) {
      ...
      if (provider === "codex") {
        try {
          const proxyUrl = appendProxyPoolQuery(new URL(`/api/oauth/codex/start-proxy`, window.location.origin));
      ...
      if (provider === "xai") {
        try {
          const proxyUrl = appendProxyPoolQuery(new URL(`/api/oauth/xai/start-proxy`, window.location.origin));
          proxyUrl.searchParams.set("app_port", appPort);
          proxyUrl.searchParams.set("state", data.state);
          proxyUrl.searchParams.set("code_verifier", data.codeVerifier);
          proxyUrl.searchParams.set("redirect_uri", redirectUri);
  ...
  return (
    <Modal isOpen={isOpen} title={modalTitle} onClose={handleClose} size="lg">
      <div className="flex flex-col gap-4">
        {(proxyPools || []).length > 0 && step !== "success" && (
          <Select
              ...
              <>
                {step === "waiting" && (
                  <div className="flex items-center gap-2 px-3 py-2 border border-border rounded-lg bg-sidebar/50">
                    <MdiIcon name="progress_activity" size={16} spin className="text-primary animate-spin" />
        ...
        {step === "error" && (
          <div className="text-center py-6">
            <div className="size-16 mx-auto mb-4 rounded-full bg-red-100 dark:bg-red-900/30 flex items-center justify-center">
              <MdiIcon name="error" size={30} className="text-red-600" />
            </div>
