...
import { Modal, Button, OAuthProxyPoolSelect, useOAuthProxyPool } from "@/shared/components";
...
export default function BulkImportGrokCliModal({ isOpen, onClose, onSuccess, proxyPools }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch("/api/oauth/grok-cli/bulk-import", {
        ...
        body: JSON.stringify({ accounts, ...proxyPoolPayload }),
      });
        ...
        >
          ...
          {isDragging && (
            <div className="absolute inset-0 flex flex-col items-center justify-center bg-sidebar/90 rounded pointer-events-none backdrop-blur-xs">
              <MdiIcon name="upload_file" size={30} className="text-primary mb-1" />
