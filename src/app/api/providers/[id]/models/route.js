...
import { resolveConnectionProxyConfig, buildProxyOptionsForConnection } from "@/lib/network/connectionProxy";
import { resolveCursorModels } from "open-sse/services/cursorModels.js";
import { providerFetch } from "@/lib/providerFetch.js";
import { logModelListRequested, logModelMetadataEnriched, logModelCatalogPersisted, buildEgressLogFields } from "@/lib/providerOperationLog.js";
import { enrichModelList } from "@/shared/utils/modelCatalog.js";
import {
  loadProviderModelCatalog,
  persistProviderModelCatalog,
  markCatalogSyncFailed,
} from "@/lib/db/repos/providerModelCatalogRepo.js";
...
const buildOAuthResolver = ({ refreshFn, fetchFn, parseFn, errorLabel }) => async (connection) => {
  ...
  const proxyOptions = await buildProxyOptionsForConnection(connection);
  let warning;
  try {
...
const PROVIDER_MODELS_CONFIG = {
  ...
  codex: {
    customResolver: buildOAuthResolver({
      refreshFn: (conn, proxyOptions) => refreshCodexToken(conn.refreshToken, console, proxyOptions),
      fetchFn: (token, _conn, proxyOptions) => providerFetch(CODEX_MODELS_URL, {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
      ...
      }, { ...proxyOptions, operation: "model_list" }),
  ...
  "gemini-cli": {
    customResolver: buildOAuthResolver({
      refreshFn: (conn, proxyOptions) => refreshGoogleToken(conn.refreshToken, GEMINI_CONFIG.clientId, GEMINI_CONFIG.clientSecret, console, proxyOptions),
      fetchFn: (token, conn, proxyOptions) => {
        const projectId = conn.projectId || conn.providerSpecificData?.projectId;
        const body = projectId ? { project: projectId } : {};
        return providerFetch(GEMINI_CLI_MODELS_URL, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            "Authorization": `Bearer ${token}`,
  ...
  "ollama-local": {
    customResolver: async (connection) => {
      ...
      const response = await providerFetch(url, {
        method: "GET",
        headers: { "Content-Type": "application/json" }
      }, { ...proxyOptions, providerId: connection.provider, connectionId: connection.id, operation: "model_list" });
      if (!response.ok) {
        const errorText = await response.text();
...
async function finishModelListResponse(connection, rawModels, op, proxyOptions, { warning } = {}) {
  const { rows, stats } = enrichModelList(rawModels, connection.provider, { source: "provider" });
  logModelMetadataEnriched({
  ...
  try {
    ...
    mergedRows = mergeDiscoveredCatalog(existing?.models || [], rows);
    const persisted = await persistProviderModelCatalog(connection, {
      models: mergedRows,
      enrichment: stats,
      syncStatus: "synced",
      requestId: op.requestId,
    });
    logModelCatalogPersisted({
      providerId: connection.provider,
      connectionId: connection.id,
      modelCount: mergedRows.length,
      catalogKey: persisted.catalogKey,
    });
  } catch (persistErr) {
    console.error("[PROVIDER] event=model_catalog_persisted error=", persistErr?.message || persistErr);
  }

  const egress = buildEgressLogFields(proxyOptions);
  op.logTerminal({
    event: "model_list_succeeded",
    ok: true,
    returnedModelCount: mergedRows.length,
...
async function failModelList(op, error, status = 500, proxyOptions, connection) {
  const egress = proxyOptions ? buildEgressLogFields(proxyOptions) : {};
  if (connection) {
    try {
      await markCatalogSyncFailed(connection, { error, requestId: op?.requestId });
    } catch { /* fail-open */ }
  }
  op?.logTerminal({
    event: "model_list_failed",
    ok: false,
    error: String(error),
    status,
    level: "error",
  ...
  return NextResponse.json({ error: String(error), requestId: op?.requestId }, { status });
}
...
export async function GET(request, { params }) {
  ...
  try {
    ...
    proxyOptions = await buildProxyOptionsForConnection(connection);
    op = logModelListRequested({
      providerId: connection.provider,
      connectionId: connection.id,
      ...buildEgressLogFields(proxyOptions),
    });

    if (isOpenAICompatibleProvider(connection.provider)) {
      const baseUrl = connection.providerSpecificData?.baseUrl;
      if (!baseUrl) {
        return failModelList(op, "No base URL configured for OpenAI compatible provider", 400, proxyOptions, connection);
      }
      ...
      }, { ...proxyOptions, providerId: connection.provider, connectionId: connection.id, operation: "model_list", requestId: op.requestId });

      if (!response.ok) {
        const errorText = await response.text();
        return failModelList(op, `Failed to fetch models: ${response.status}`, response.status, proxyOptions, connection);
      }

      const data = await response.json();
      const models = data.data || data.models || [];
      return finishModelListResponse(connection, models, op, proxyOptions);
    }

    if (isAnthropicCompatibleProvider(connection.provider)) {
      let baseUrl = connection.providerSpecificData?.baseUrl;
      if (!baseUrl) {
        return failModelList(op, "No base URL configured for Anthropic compatible provider", 400, proxyOptions, connection);
      }
    ...
    if (!config) {
      return failModelList(op, `Provider ${connection.provider} does not support models listing`, 400, proxyOptions, connection);
    }

    if (typeof config.customResolver === "function") {
      const result = await config.customResolver(connection);
      if (result.error) {
        return failModelList(op, result.error, result.status || 500, proxyOptions, connection);
      }
      return finishModelListResponse(connection, result.models || [], op, proxyOptions, { warning: result.warning });
    }

    const token = connection.providerSpecificData?.copilotToken || connection.accessToken || connection.apiKey;
    if (!token) {
      return failModelList(op, "No valid token found", 401, proxyOptions, connection);
    }
    ...
    const response = await providerFetch(url, fetchOptions, {
      ...proxyOptions,
      providerId: connection.provider,
      connectionId: connection.id,
    ...
    if (!response.ok) {
      await response.text();
      return failModelList(op, `Failed to fetch models: ${response.status}`, response.status, proxyOptions);
    }
    ...
    return failModelList(op, error.message || "Failed to fetch models", 500, proxyOptions, connection);
  }
}
