...
export default function JcodeToolCard({
  ...
  const hairouterProvider = pickProviderBlock(jcodeStatus?.config?.providers);
  const currentBaseUrl = hairouterProvider?.base_url || "";

  const getConfigStatus = () => {
    if (!jcodeStatus?.installed) return null;
    if (!isHairouterConfigured(jcodeStatus)) return "not_configured";
    const currentProvider = pickProviderBlock(jcodeStatus.config?.providers);
  ...
  useEffect(() => {
    if (jcodeStatus?.installed && !hasInitializedModel.current) {
      ...
      const provider = pickProviderBlock(jcodeStatus.config?.providers);
      if (provider) {
  ...
  const getManualConfigs = () => {
    ...
    const configToml = `[providers.${CLI_PROVIDER_SLUG}]
...
[[providers.${CLI_PROVIDER_SLUG}.models]]
id = "${selectedModel || "cc/claude-opus-4-7"}"`;

    const envContent = `${API_KEY_ENV}="${keyToUse}"`;
    ...
    return [
      ...
      {
        filename: `~/.config/jcode/${PROVIDER_ENV_FILE}`,
        content: envContent,
      },
    ];
  };
              ...
              <div className="flex flex-col gap-3 p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-text-muted mt-2">Manual configuration is still available if HAI-Router is deployed on a remote server.</p>
                  </div>
                </div>
            ...
            <>
              <div className="flex flex-col gap-2">
                  ...
                  <div className="flex flex-col gap-2 mb-2">
                    {tool.notes.map((note, idx) => (
                      <div key={idx} className={`flex items-start gap-2 p-2 rounded text-xs ${
                        ...
                        <MdiIcon
                          name={note.type === "info" ? "info" : note.type === "warning" ? "warning" : "help"}
                          size={14}
                          className="mt-0.5"
                        />
                        <span>{note.text}</span>
                ...
                <div className="flex flex-col gap-1 p-3 bg-blue-500/5 border border-blue-500/20 rounded-lg">
                  ...
                  <code className="text-xs font-mono text-text-muted">jcode --provider-profile {CLI_PROVIDER_SLUG}</code>
                  <code className="text-xs font-mono text-text-muted">jcode --provider-profile {CLI_PROVIDER_SLUG} --model {selectedModel || "cc/claude-opus-4-7"}</code>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                <Button variant="primary" size="sm" onClick={handleApplySettings} disabled={!selectedModel} loading={applying}>
                  <MdiIcon name="save" size={14} className="mr-1" />Apply
                </Button>
                <Button variant="outline" size="sm" onClick={handleResetSettings} disabled={!isHairouterConfigured(jcodeStatus)} loading={restoring}>
                  <MdiIcon name="restore" size={14} className="mr-1" />Reset
                </Button>
