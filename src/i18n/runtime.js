...
function processTextNode(node) {
  ...
  // Store / refresh original text when React updates copy (avoid stale header on navigation)
  const current = node.nodeValue;
  const translatedFromStored =
    node._originalText != null ? translate(node._originalText) : null;

  if (node._originalText == null) {
    node._originalText = current;
  } else if (current !== node._originalText && current !== translatedFromStored) {
    node._originalText = current;
  }
