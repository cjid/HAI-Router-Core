...
import { CLI_PROVIDER_SLUG } from "./hairouterCliHelpers";
import { isHairouterConfigured } from "./hairouterCliHelpers";
import { APP_CONFIG } from "@/shared/constants/config";

const ENDPOINT = "/api/cli-tools/grok-build-settings";
const MODEL_SLOT = CLI_PROVIDER_SLUG;
    ...
    <Card padding="xs" className="overflow-hidden">
      ...
      {isExpanded && (
        <div className="mt-4 pt-4 border-t border-border flex flex-col gap-4">
          {checking && <div className="flex items-center gap-2 text-text-muted"><MdiIcon name="progress_activity" size={18} spin className="animate-spin" /><span>Checking Grok Build...</span></div>}
          ...
          {!checking && grokStatus && !grokStatus.installed && (
            <div className="flex flex-col gap-3 p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
              ...
              <Button variant="secondary" size="sm" onClick={() => setShowManualConfigModal(true)} className="w-full sm:w-auto"><MdiIcon name="content_copy" size={18} className="mr-1" />Manual Config</Button>
            </div>
          )}
          ...
          {!checking && grokStatus?.installed && (
            <>
              <div className="flex flex-col gap-2">
                  ...
                  <div className="mb-2 flex flex-col gap-2">
                    {tool.notes.map((note, index) => (
                      <div key={index} className={`flex items-start gap-2 rounded p-2 text-xs ${note.type === "warning" ? "bg-yellow-500/10 text-yellow-600 dark:text-yellow-400" : "bg-blue-500/10 text-blue-600 dark:text-blue-400"}`}>
                        <MdiIcon name={note.type === "warning" ? "warning" : "info"} size={14} className="mt-0.5" />
                        <span>{note.text}</span>
                      </div>
                    ))}
                  </div>
                ...
                <div className="my-1 border-t border-border pt-3">
                  <div className="mb-2 flex items-start gap-2">
                    <MdiIcon name="account_tree" size={16} className="text-primary" />
                    <div>
              ...
              {message && <div className={`flex items-center gap-2 px-2 py-1.5 rounded text-xs ${message.type === "success" ? "bg-green-500/10 text-green-600" : "bg-red-500/10 text-red-600"}`}><MdiIcon name={message.type === "success" ? "check_circle" : "error"} size={14} /><span>{message.text}</span></div>}
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                ...
                <Button
                  ...
                  size="sm"
                  onClick={handleReset}
                  disabled={!isHairouterConfigured(grokStatus)}
                  processState={resetProcess.processState}
                  processLabels={PROCESS_LABEL_PRESETS.reset}
                  icon="restore"
                  className="w-full sm:w-auto"
                />
                <Button variant="ghost" size="sm" onClick={() => setShowManualConfigModal(true)} className="w-full sm:w-auto"><MdiIcon name="content_copy" size={14} className="mr-1" />Manual Config</Button>
              </div>
            </>
          )}
        </div>
