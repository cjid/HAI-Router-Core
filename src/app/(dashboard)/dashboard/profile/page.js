...
import { Card, Button, Toggle, Input, SettingsSkeleton, Select } from "@/shared/components";
...
import { PRODUCT, resolveSamlIssuer } from "@/shared/constants/product.js";
import { PROCESS_LABEL_PRESETS, PROCESS_STATE } from "@/shared/constants/buttonProcess";
import { LOCALE_COOKIE, normalizeLocale } from "@/i18n/config";
import { LOCALE_FLAGS } from "@/shared/constants/locales";
import { TIMEZONE_OPTIONS, DEFAULT_TIMEZONE, DEFAULT_HOUR_FORMAT } from "@/shared/constants/timezones";
...
export default function ProfilePage() {
  ...
  const [displayDbPath, setDisplayDbPath] = useState("~/.hairouter/db/data.sqlite");
  ...
  const [samlForm, setSamlForm] = useState({
    samlEntryPoint: "",
    samlIssuer: PRODUCT.samlIssuerDefault,
    samlCert: "",
  ...
  useEffect(() => {
    useSettingsStore
      .getState()
      .fetchSettings({ signal: navSignal })
      .then((data) => {
        if (!data) return;
        setSettings(data);
        setOidcForm({
        ...
        setSamlForm({
          ...
          samlIssuer: data?.samlIssuer || PRODUCT.samlIssuerDefault,
  ...
  useEffect(() => {
    fetch("/api/settings/data-path")
      .then((res) => res.json())
      .then((data) => {
        if (data?.displayDbPath) setDisplayDbPath(data.displayDbPath);
      })
      .catch(() => {});
  }, []);
    ...
    reader.onload = (e) => {
      try {
        ...
        setSamlForm((prev) => ({
          ...
          samlIssuer: prev.samlIssuer || PRODUCT.samlIssuerDefault,
  ...
  const saveSamlSettings = async (targetAuthMode = oidcForm.authMode || "password") => {
    ...
    try {
      const payload = {
        ...
        samlIssuer: samlForm.samlIssuer.trim() || PRODUCT.samlIssuerDefault,
  ...
  const handleExportDatabase = async (password) => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `hairouter-backup-${stamp}.json`;
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 sm:px-0">
      <div className="flex flex-col gap-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
            <div className="flex items-center gap-3 sm:gap-4">
              <div className="size-10 sm:size-12 rounded-lg bg-green-500/10 text-green-500 flex items-center justify-center shrink-0">
                <MdiIcon name="computer" size={20} className="sm:text-2xl" />
              </div>
              <div>
                ...
                <p className="text-sm text-text-muted">Data remains on this machine</p>
              </div>
            </div>
            <div>
              <p className="mb-1.5 text-xs font-medium text-text-muted">Appearance</p>
          ...
          <div className="flex flex-col gap-3 pt-4 border-t border-border">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between p-3 rounded-lg bg-bg border border-border gap-2">
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs sm:text-sm text-text-muted font-mono break-all">{displayDbPath}</p>
              </div>
              <Button
                variant="secondary"
                size="sm"
                icon="content_copy"
                onClick={() => navigator.clipboard?.writeText(displayDbPath)}
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              <Button
                ...
                processState={dbLoading ? PROCESS_STATE.EXPORTING : PROCESS_STATE.IDLE}
                processLabels={PROCESS_LABEL_PRESETS.exportBackup}
        ...
        <Card>
          <div className="flex items-center gap-3 mb-4">
            <div className="size-10 rounded-lg bg-violet-500/10 text-violet-500 flex items-center justify-center shrink-0">
              <MdiIcon name="schedule" size={20} />
            </div>
            <div>
              <h3 className="text-base sm:text-lg font-semibold">Date &amp; time</h3>
              <p className="text-xs sm:text-sm text-text-muted">Applies to timestamps across the dashboard</p>
            </div>
          </div>
          <div className="flex flex-col gap-4">
            <Select
              ...
              onChange={async (e) => {
                ...
                const updated = await useSettingsStore.getState().patchSettings({ timezone });
                if (updated) setSettings((prev) => ({ ...prev, timezone }));
              }}
            />
            <Select
              label="Time format"
              ...
              variant="descriptive"
              fullWidth
              value={settings.hourFormat || DEFAULT_HOUR_FORMAT}
              options={[
                { value: "24", label: "24-hour", description: "Example: 23:45" },
                { value: "12", label: "12-hour", description: "Example: 11:45 PM" },
              ]}
              onChange={async (e) => {
                const hourFormat = e.target.value;
                const updated = await useSettingsStore.getState().patchSettings({ hourFormat });
                if (updated) setSettings((prev) => ({ ...prev, hourFormat }));
              }}
            />
          </div>
        </Card>
              ...
              <form onSubmit={handlePasswordChange} className="flex flex-col gap-4 pt-4 border-t border-border/50">
                ...
                <div className="pt-2">
                  <Button
                    type="submit"
                    variant="primary"
                    processState={passLoading ? PROCESS_STATE.SAVING : PROCESS_STATE.IDLE}
                    processLabels={{
                      idle: settings.hasPassword ? "Update Password" : "Set Password",
                      saving: settings.hasPassword ? "Updating" : "Setting",
                      success: settings.hasPassword ? "Updated" : "Set",
                      error: settings.hasPassword ? "Update Password" : "Set Password",
                    }}
                    className="w-full sm:w-auto"
                  />
                </div>
        ...
        <Card>
          ...
          >
            ...
            <MdiIcon
              name={oidcExpanded ? "expand_less" : "expand_more"}
              size={18}
                ...
                <div className="flex flex-col gap-4 pt-2 border-t border-border/50">
                  ...
                  <div className="rounded-lg border border-border bg-bg/80 overflow-hidden">
                    ...
                    >
                      <div className="flex items-center gap-2">
                        <MdiIcon name="menu_book" size={18} className="text-primary" />
                        <div>
                        ...
                        <div className="p-2.5 rounded border border-primary/20 bg-primary/5 text-primary text-xs">
                          ...
                          <ul className="list-disc pl-4 space-y-1 font-mono text-[11px]">
                            ...
                            <li>
                              ...
                              <code className="bg-bg px-1 py-0.5 rounded break-all">{samlForm.samlIssuer || PRODUCT.samlIssuerDefault}</code>
                            </li>
                            <li>
                              <b>NameID Format:</b>{" "}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-1">
                          <div className="p-3 rounded border border-border bg-bg/50 flex flex-col gap-1.5">
                            ...
                            <ol className="list-decimal pl-4 text-text-muted space-y-1">
                              ...
                              <li>Set <b>Application SAML audience</b> to <code className="text-text-main font-mono">{samlForm.samlIssuer || PRODUCT.samlIssuerDefault}</code>.</li>
                          ...
                          <div className="p-3 rounded border border-border bg-bg/50 flex flex-col gap-1.5">
                            ...
                            <ol className="list-decimal pl-4 text-text-muted space-y-1">
                              ...
                              <li><b>Identifier (Entity ID):</b> <code className="text-text-main font-mono">{samlForm.samlIssuer || PRODUCT.samlIssuerDefault}</code></li>
                          ...
                          <div className="p-3 rounded border border-border bg-bg/50 flex flex-col gap-1.5">
                            ...
                            <ol className="list-decimal pl-4 text-text-muted space-y-1">
                              ...
                              <li><b>Audience URI (SP Entity ID):</b> <code className="text-text-main font-mono">{samlForm.samlIssuer || PRODUCT.samlIssuerDefault}</code></li>
                          ...
                          <div className="p-3 rounded border border-border bg-bg/50 flex flex-col gap-1.5">
                            ...
                            <ol className="list-decimal pl-4 text-text-muted space-y-1">
                              ...
                              <li><b>Client ID:</b> <code className="text-text-main font-mono">{samlForm.samlIssuer || PRODUCT.samlIssuerDefault}</code></li>
                  ...
                  <div className="grid grid-cols-1 gap-4">
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <Input
                        placeholder={PRODUCT.samlIssuerDefault}
                  ...
                  <div className="flex flex-col gap-2 p-3 rounded-lg border border-border bg-bg text-xs sm:text-sm text-text-muted">
                    ...
                    <div className="flex items-center justify-between gap-2 pt-2 border-t border-border/50">
                      ...
                      <a
                        ...
                        download="hairouter-sp-metadata.xml"
                  ...
                  <div className="flex flex-col sm:flex-row gap-2 pt-2 border-t border-border/50">
                    <Button
                      type="button"
                      variant="primary"
                      processState={samlLoading ? PROCESS_STATE.SAVING : PROCESS_STATE.IDLE}
                      processLabels={{
                        idle: "Save SAML settings",
                        saving: "Saving",
                        success: "Saved",
                        error: "Save SAML settings",
                      }}
                    ...
                    <Button
                      type="button"
                      variant="outline"
                      processState={samlTestLoading ? PROCESS_STATE.TESTING : PROCESS_STATE.IDLE}
                      processLabels={{
                        idle: "Test SAML settings",
                        testing: "Testing",
                        success: "Passed",
                        error: "Test SAML settings",
                      }}
                ...
                <div className="flex flex-col gap-4 pt-2 border-t border-border/50">
                  <div className="grid grid-cols-1 gap-4">
                    <div className="flex flex-col gap-2">
                      ...
                      <Input
                        placeholder="https://auth.example.com/application/o/hairouter/"
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <Input
                        placeholder="hairouter-dashboard"
                  ...
                  <div className="flex flex-col sm:flex-row gap-2 pt-2 border-t border-border/50">
                    <Button
                      type="button"
                      variant="primary"
                      processState={oidcLoading ? PROCESS_STATE.SAVING : PROCESS_STATE.IDLE}
                      processLabels={{
                        idle: "Save OIDC settings",
                        saving: "Saving",
                        success: "Saved",
                        error: "Save OIDC settings",
                      }}
                    ...
                    <Button
                      type="button"
                      variant="outline"
                      processState={oidcTestLoading ? PROCESS_STATE.TESTING : PROCESS_STATE.IDLE}
                      processLabels={{
              ...
              <form onSubmit={updateOutboundProxy} className="flex flex-col gap-4 pt-2 border-t border-border/50">
                ...
                <div className="pt-2 border-t border-border/50 flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
                  <Button
                    type="button"
                    variant="secondary"
                    processState={proxyTestLoading ? PROCESS_STATE.TESTING : PROCESS_STATE.IDLE}
                    processLabels={PROCESS_LABEL_PRESETS.testProxy}
                    disabled={loading || proxyLoading}
                  ...
                  <Button
                    type="submit"
                    variant="primary"
                    processState={proxyLoading ? PROCESS_STATE.SAVING : PROCESS_STATE.IDLE}
                    processLabels={PROCESS_LABEL_PRESETS.apply}
                    icon="save"
                    className="w-full sm:w-auto"
                  />
                </div>
        ...
        <Card>
          <div className="flex items-center gap-3 mb-4">
            <div className="p-2 rounded-lg bg-orange-500/10 text-orange-500 shrink-0">
              <MdiIcon name="monitoring" size={20} />
            </div>
        ...
        footer={
          <>
            ...
            <Button
              ...
              processState={dbLoading ? PROCESS_STATE.LOADING : PROCESS_STATE.IDLE}
              processLabels={PROCESS_LABEL_PRESETS.confirm}
