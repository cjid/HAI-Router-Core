...
/**
 * Model-test probes should always record non-zero input tokens so Recent Requests
 * stays reliable when upstream omits usage metadata.
 */
export function ensureModelTestUsage(tokens, { body = null, contentLength = 0 } = {}) {
  const base = tokens && typeof tokens === "object"
    ? { ...tokens }
    : { prompt_tokens: 0, completion_tokens: 0 };

  let inTok = base.prompt_tokens ?? base.input_tokens ?? 0;
  let outTok = base.completion_tokens ?? base.output_tokens ?? 0;
  if (inTok > 0 && outTok > 0 && !base.estimated) return base;

  const estimated = estimateUsage(body, contentLength);
  const merged = mergeUsage(base, estimated) || { ...estimated };

  inTok = merged.prompt_tokens ?? merged.input_tokens ?? 0;
  if (inTok <= 0) {
    const promptTokens = Math.max(1, body ? estimateInputTokens(body) : 1);
    if (merged.input_tokens !== undefined) merged.input_tokens = promptTokens;
    else merged.prompt_tokens = promptTokens;
  }

  outTok = merged.completion_tokens ?? merged.output_tokens ?? 0;
  if (outTok <= 0 && contentLength > 0) {
    const completionTokens = Math.max(1, estimateOutputTokens(contentLength));
    if (merged.output_tokens !== undefined) merged.output_tokens = completionTokens;
    else merged.completion_tokens = completionTokens;
  }
...
/** Rough output char length from raw upstream SSE bytes (JSON/event framing overhead). */
export function estimateContentLengthFromBytes(totalBytes) {
  if (!totalBytes || totalBytes <= 0) return 0;
  return Math.max(1, Math.floor(totalBytes / 8));
}

/**
 * Ensure partial/aborted streams still have auditable usage (never silent zero).
 * Provider-reported usage is preserved; gaps are filled with honest estimates.
 */
export function ensurePartialUsage(usage, {
  body = null,
  contentLength = 0,
  totalBytes = 0,
  sawToolCalls = false,
  ...
  const hadStreamActivity = totalBytes > 0 || chunkCount > 0;
  if (!isPartial && !hadStreamActivity && hasValidUsage(usage) && !usage?.estimated) return usage;

  let outLen = contentLength || 0;
  if (outLen === 0 && totalBytes > 0) {
    outLen = estimateContentLengthFromBytes(totalBytes);
  }
  if (outLen === 0 && sawToolCalls && totalBytes > 0) {
    outLen = Math.max(1, Math.floor(totalBytes / 12));
  }

  if (hasValidUsage(usage) && !usage?.estimated) {
    const outTok = usage.completion_tokens ?? usage.output_tokens ?? 0;
    if (outTok > 0 || outLen === 0) return usage;
    const est = estimateUsage(body, outLen, targetFormat);
    return mergeUsage(usage, { ...est, estimated: true });
  }

  if (outLen > 0 || body) {
    return estimateUsage(body, outLen, targetFormat);
  }
  if (hadStreamActivity) {
    return estimateUsage(null, outLen, targetFormat);
  }
  return usage;
}
