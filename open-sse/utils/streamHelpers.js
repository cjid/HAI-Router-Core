...
export function parseSSELine(line, format = null) {
  ...
  // NDJSON format (Ollama and similar): raw JSON lines without "data:" prefix
...
// Check if chunk has valuable content (not empty) — includes role-only for relay
...
/** Stricter TTFT check — excludes role-only / heartbeat / empty metadata deltas. */
export function hasMeaningfulTokenContent(chunk, format) {
  if (format === FORMATS.OPENAI && chunk.choices?.[0]?.delta) {
    const delta = chunk.choices[0].delta;
    return Boolean(
  ...
  if (format === FORMATS.CLAUDE) {
    if (chunk.type === "content_block_delta") {
      ...
      return Boolean(
        ...
        || (d?.partial_json && d.partial_json !== ""),
      );
    }
    return Boolean(chunk.type === "content_block_start" && chunk.content_block?.type === "text");
  }
  return Boolean(hasValuableContent(chunk, format));
}
