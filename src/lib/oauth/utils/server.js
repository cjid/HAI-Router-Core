      ...
      if (session) {
        try {
          ...
          const tokenData = await exchangeTokens(
            ...
            state,
            session.proxyPoolId ? { proxyPoolId: session.proxyPoolId } : undefined,
          );
...
export function registerTraeSession({ state, proxyPoolId }) {
  if (!state) return false;
  traeSession = { state, proxyPoolId: proxyPoolId || null, status: "pending", createdAt: Date.now() };
  return true;
}
...
export function registerWindsurfSession({ state, proxyPoolId }) {
  if (!state) return false;
  windsurfSession = { state, proxyPoolId: proxyPoolId || null, status: "pending", createdAt: Date.now() };
  return true;
}
...
export function registerZedSession({ state, codeVerifier, proxyPoolId }) {
  if (!state || !codeVerifier) return false;
  zedSession = { state, codeVerifier, proxyPoolId: proxyPoolId || null, status: "pending", createdAt: Date.now() };
  return true;
}
