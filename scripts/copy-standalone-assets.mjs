...
export function copyStandaloneAssets({ projectRoot = process.cwd(), distDir = process.env.NEXT_DIST_DIR || ".next" } = {}) {
  ...
  const goBinSource = resolve(projectRoot, "go-engine", "bin");
  const goBinDestination = resolve(standaloneDir, "go-engine", "bin");
  if (existsSync(goBinSource)) {
    cpSync(goBinSource, goBinDestination, { recursive: true, force: true });
    console.log(`[standalone-assets] Copied Go worker binaries to ${goBinDestination}`);
  }
}
