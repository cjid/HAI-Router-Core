...
const INSTALL_LOG_TAIL = 8000;
const INSTALL_LOG_POLL_MS = 2500;
/** PXPIPE card is hidden — skip status/health probes that block navigation. */
const PXPIPE_UI_ENABLED = false;
const HEADROOM_STATUS_DEFER_MS = 100;
const HEADROOM_EXTRAS_AVAILABLE = ["code", "ml"];

const InstallLogPanel = memo(function InstallLogPanel({ log }) {
  if (!log) return null;
  return (
...
export default function TokenSaverClient() {
  ...
  const applyHeadroomExtrasFromStatus = useCallback((data) => {
    setHeadroomExtras({
      version: data?.version ?? null,
      extras: data?.extras || { code: false, ml: false },
      available: HEADROOM_EXTRAS_AVAILABLE,
      loading: false,
    });
    setPendingExtras([]);
  }, []);

  const refreshHeadroomStatus = useCallback(async ({ silent = false, signal } = {}) => {
    if (!silent) {
    ...
    try {
      ...
      // /api/headroom/status already includes version + extras — avoid a second pip probe.
      applyHeadroomExtrasFromStatus(data);
    } catch (error) {
  ...
  const handleHeadroomStart = useCallback(async () => {
    ...
    try {
      ...
      await refreshHeadroomStatus({ silent: true });
    } catch (e) {
      if (mountedRef.current) setHeadroomActionError(e.message);
    } finally {
      if (mountedRef.current) setHeadroomActionLoading(false);
    }
  }, [refreshHeadroomStatus]);
  ...
  const startLogPolling = useCallback(() => {
    ...
    const pollSignal = fetchAbortRef.current?.signal;
    const tick = async () => {
      if (!mountedRef.current || pollSignal?.aborted) return;
      try {
        ...
        if (typeof d.log !== "string") return;
        const next = d.log.slice(-INSTALL_LOG_TAIL);
        if (next === lastLogRef.current) return;
        lastLogRef.current = next;
        startLogTransition(() => setInstallLog(next));
      } catch (error) {
        if (error?.name === "AbortError") return;
      }
    };
    tick();
    logPollRef.current = setInterval(tick, INSTALL_LOG_POLL_MS);
  }, [startLogTransition]);
  ...
  const installExtrasConfirmed = useCallback(async () => {
    ...
    try {
      ...
      if (mountedRef.current) setExtrasActionError(e.message);
    } finally {
      stopLogPolling();
      if (mountedRef.current) setExtrasActionLoading(false);
    }
  ...
  const removeExtraConfirmed = useCallback(async (extra) => {
    ...
    try {
      ...
      stopLogPolling();
      if (mountedRef.current) setRemovingExtra(null);
    }
  ...
  const toggleExtraActive = useCallback(async (extra, value) => {
    ...
    try {
      ...
      if (mountedRef.current) setRestartingProxy(false);
    }
      ...
      try {
        ...
        if (res.ok) {
          ...
          deferTimerRef.current = setTimeout(() => {
            if (!mountedRef.current || signal?.aborted) return;
            refreshHeadroomStatus({ signal });
          }, HEADROOM_STATUS_DEFER_MS);
          if (PXPIPE_UI_ENABLED) {
            refreshPxpipeStatus().then(() => {
              if (!mountedRef.current || signal?.aborted) return;
              runPxpipeHealth();
            });
          }
  ...
  }, [refreshHeadroomStatus, refreshPxpipeStatus, runPxpipeHealth, stopLogPolling, navSignal]);
      ...
      <Card id="rtk">
        <div className="flex items-center justify-between mb-2">
          <h2 className="text-lg font-semibold flex items-center gap-2">
            <MdiIcon name={"bolt"} size={18} className="text-primary" />
            Token Saver
          </h2>
        </div>
          ...
          {!pxpipeStatus.installed ? (
            <div className="flex flex-col gap-2">
              ...
              <p className="text-xs text-text-muted">
                ...
                the HAI-Router data directory. May take a few minutes.
              </p>
            </div>
          ) : (
