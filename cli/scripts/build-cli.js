...
function buildCliPackage() {
  ...
  // App version SSOT is hairouter-app package.json (0.1.0-init). CLI npm artifact
  // (9router@0.5.59) is separate — do not overwrite app version from cli/package.json.
  console.log("0️⃣  App version (SSOT):");
  const appPkgPath = path.join(appDir, "package.json");
  const appPkg = JSON.parse(fs.readFileSync(appPkgPath, "utf8"));
  const cliPkg = JSON.parse(fs.readFileSync(path.join(cliDir, "package.json"), "utf8"));
  console.log(`   hairouter-app ${appPkg.version} · cli npm ${cliPkg.name}@${cliPkg.version}\n`);

  // Sync data-dir SSOT into CLI hooks (published tarball has no ../src).
  fs.copyFileSync(
    path.join(appDir, "src", "lib", "dataDirCore.cjs"),
    path.join(cliDir, "hooks", "dataDirCore.cjs"),
  );

  // Step 0b: Build Go worker binaries for packaged CLI distribution
  console.log("0️⃣ b Building Go worker binaries...");
  try {
    execSync("npm run build:go-engine:all", { stdio: "inherit", cwd: appDir });
    console.log("✅ Go worker binaries built\n");
  } catch (error) {
    console.error("❌ Go worker build failed");
    process.exit(1);
  }
  ...
  if (fs.existsSync(betterDir)) {
    ...
    console.log("✅ Stripped better-sqlite3 (lives in ~/.hairouter/runtime)");
  }
  console.log("");
  ...
  // Step 7c: Copy Go worker binaries (not traced by Next.js standalone)
  console.log("7️⃣ c Copying Go worker binaries...");
  const goBinSrc = path.join(appDir, "go-engine", "bin");
  const goBinDest = path.join(cliAppDir, "go-engine", "bin");
  if (fs.existsSync(goBinSrc)) {
    copyRecursive(goBinSrc, goBinDest);
    console.log("✅ Copied Go worker binaries\n");
  } else {
    console.error("❌ go-engine/bin not found — run build:go-engine:all first");
    process.exit(1);
  }
