...
export default {
  synthesize(text, model, credentials, responseFormat, { style, language } = {}) {
    ...
    return synthesizeMiMo(text, model, credentials.apiKey, style, language, credentials);
  },
};

export async function synthesizeMiMo(text, model, apiKey, style, language, credentials = null) {
  ...
  const res = await modalityFetch("https://api.xiaomimimo.com/v1/chat/completions", {
    method: "POST",
    headers: {
