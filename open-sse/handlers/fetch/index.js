...
async function tryFetch(url, init, timeoutMs, credentials) {
  const ctrl = new AbortController();
  const timer = setTimeout(() => ctrl.abort(), timeoutMs);
  try {
    const res = await modalityFetch(url, { ...init, headers: sanitizeHeaders(init.headers), signal: ctrl.signal }, credentials);
    return { ok: true, res };
  } catch (err) {
...
export async function handleFetchCore({ url, format, maxCharacters, provider, providerConfig, credentials, log }) {
  ...
  try {
    if (provider === "firecrawl") {
      return await runFirecrawl({ url, fmt, timeoutMs, apiKey, maxCharacters, costPerQuery, startedAt, credentials });
    }
    if (provider === "jina-reader") {
      return await runJina({ url, fmt, timeoutMs, apiKey, maxCharacters, costPerQuery, startedAt, credentials });
    }
    if (provider === "tavily") {
      return await runTavily({ url, fmt, timeoutMs, apiKey, maxCharacters, costPerQuery, startedAt, credentials });
    }
    if (provider === "exa") {
      return await runExa({ url, fmt, timeoutMs, apiKey, maxCharacters, costPerQuery, startedAt, credentials });
    }
...
async function runFirecrawl({ url, fmt, timeoutMs, apiKey, maxCharacters, costPerQuery, startedAt, credentials }) {
...
async function runJina({ url, fmt, timeoutMs, apiKey, maxCharacters, costPerQuery, startedAt, credentials }) {
  const upstreamStart = Date.now();
...
async function runTavily({ url, fmt, timeoutMs, apiKey, maxCharacters, costPerQuery, startedAt, credentials }) {
  const upstreamStart = Date.now();
...
async function runExa({ url, fmt, timeoutMs, apiKey, maxCharacters, costPerQuery, startedAt, credentials }) {
  const upstreamStart = Date.now();
