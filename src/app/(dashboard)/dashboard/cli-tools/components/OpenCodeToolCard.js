...
export default function OpenCodeToolCard({ tool, isExpanded, onToggle, baseUrl, apiKeys, activeProviders, cloudEnabled, initialStatus, tunnelEnabled, tunnelPublicUrl, tailscaleEnabled, tailscaleUrl }) {
  ...
  useEffect(() => {
    ...
    if (isHairouterPrefixedModel(status?.config?.agent?.explorer?.model)) {
      setSubagentModel(stripHairouterModelPrefix(status.config.agent.explorer.model) || "");
    }
  }, [status]);
  ...
  const saveModels = async (models) => {
    try {
      const keyToUse = (selectedApiKey && selectedApiKey.trim())
        ? selectedApiKey
        : (!cloudEnabled ? "sk_hairouter" : selectedApiKey);
  ...
  const hairouterProvider = pickProviderBlock(status?.config?.provider);
  const currentBaseUrl = hairouterProvider?.options?.baseURL || "";

  const getConfigStatus = () => {
    if (!status?.installed) return null;
    if (!status.config) return "not_configured";
    if (!isHairouterConfigured(status)) return "not_configured";
    const url = pickProviderBlock(status.config?.provider)?.options?.baseURL || "";
  ...
  const handleApply = () => applyProcess.run(async () => {
    setMessage(null);
    try {
      const keyToUse = (selectedApiKey && selectedApiKey.trim())
        ? selectedApiKey
  ...
  const handleReset = () => resetProcess.run(async () => {
    setMessage(null);
    try {
  ...
  const getManualConfigs = () => {
    ...
    return [{
      ...
      content: JSON.stringify({
        ...
        model: hairouterModelId(activeModelToShow),
        agent: {
          explorer: {
              ...
              <div className="flex flex-col gap-3 p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
                ...
                <div className="flex items-center gap-2 pl-9">
                  ...
                  <Button variant="outline" size="sm" onClick={() => setShowInstallGuide(!showInstallGuide)}>
                    <MdiIcon name={showInstallGuide ? "expand_less" : "help"} size={18} className="mr-1" />
          ...
          {!checking && status?.installed && (
            <>
              <div className="flex flex-col gap-2">
                ...
                {/* Current configured */}
                {hairouterProvider?.options?.baseURL && (
                ...
                <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr] sm:items-start sm:gap-2">
                  ...
                  <MdiIcon name="arrow_forward" size={14} className="text-text-muted mt-1.5" />
                    ...
                    <div className="flex flex-wrap gap-1.5 min-h-[28px] px-2 py-1.5 bg-surface rounded border border-border">
                      {selectedModels.length === 0 ? (
                        ...
                        selectedModels.map((model) => (
                          ...
                          >
                            {model === activeModel && <MdiIcon name="star" size={10} />}
                            {model}
                            <button
                              onClick={async (e) => {
                                e.stopPropagation();
                            ...
                            >
                              <MdiIcon name="close" size={18} className="text-[12px]" />
                            </button>
                          </span>
                        ))
                      )}
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                ...
                <Button
                  ...
                  size="sm"
                  onClick={handleReset}
                  disabled={!isHairouterConfigured(status)}
