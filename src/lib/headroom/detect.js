...
let headroomBinaryCache = { value: undefined, at: 0 };
let python310Cache = { value: undefined, at: 0 };

function readDetectCache(entry) {
  if (entry.value !== undefined && Date.now() - entry.at < DETECT_CACHE_MS) {
    return entry.value;
  }
  return undefined;
}

function writeDetectCache(entry, value) {
  entry.value = value;
  entry.at = Date.now();
  return value;
}
...
export function invalidateHeadroomDetectCache() {
  headroomBinaryCache = { value: undefined, at: 0 };
  python310Cache = { value: undefined, at: 0 };
}
...
export function findHeadroomBinary() {
  const cached = readDetectCache(headroomBinaryCache);
  ...
  try {
    ...
    return writeDetectCache(headroomBinaryCache, out ? out.split(/\r?\n/)[0].trim() : null);
  } catch {
    return writeDetectCache(headroomBinaryCache, null);
  }
}
...
export function findPython310() {
  const cached = readDetectCache(python310Cache);
  if (cached !== undefined) return cached;
  let fallback = null;
  for (const candidate of pythonCandidates()) {
    try {
      ...
      try {
        ...
        return writeDetectCache(python310Cache, candidate);
      } catch {
  ...
  return writeDetectCache(python310Cache, fallback);
}
