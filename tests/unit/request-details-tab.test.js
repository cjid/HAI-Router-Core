...
// Token helpers live in src/shared/utils/requestDetailTokens.js
import {
  getCachedTokens,
  getCacheCreationTokens,
  getInputTokens,
} from "@/shared/utils/requestDetailTokens.js";
...
describe("token helpers — render-time crash safety", () => {
  ...
  it("cached via prompt_tokens_details nested field", () => {
    expect(getCachedTokens({ prompt_tokens_details: { cached_tokens: 128 } })).toBe(128);
  });
...
describe("getRequestDetailById — API latency baseline", () => {
  it("warm indexed lookup reports P50/P95 (debug)", async () => {
    const payload = {
      id: "perf-warm-1",
      provider: "openai",
      model: "gpt-4",
    ...
    const got = await db.getRequestDetailById("perf-warm-1");
    expect(got?.id).toBe("perf-warm-1");
    const serialized = JSON.stringify(got);

    const samples = [];
    for (let i = 0; i < 40; i += 1) {
      const t0 = performance.now();
      await db.getRequestDetailById("perf-warm-1");
    ...
    console.log("[request-detail-perf]", JSON.stringify({
    ...
    expect(serialized.length).toBeLessThan(50 * 1024);
  });
});
