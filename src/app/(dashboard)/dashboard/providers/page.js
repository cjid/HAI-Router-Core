...
export default function ProvidersPage() {
    ...
    const fetchData = async () => {
      try {
        const [connectionsRes, nodesRes] = await Promise.all([
          fetch("/api/providers", { signal: navSignal }),
          fetch("/api/provider-nodes", { signal: navSignal }),
        ]);
        const connectionsData = await connectionsRes.json();
        const nodesData = await nodesRes.json();
        if (connectionsRes.ok) {
          setConnections(connectionsData.connections || []);
          setProviderStates(connectionsData.providerStates || {});
        }
  ...
  const getProviderStats = (providerId, authType) => {
    ...
    const providerDisabled = providerStates[providerId] === false;
    const allDisabled =
      providerDisabled ||
  ...
  const handleToggleProvider = async (providerId, authType, newActive) => {
    ...
    setProviderStates((prev) => ({ ...prev, [providerId]: newActive }));
    setConnections((prev) =>
    ...
    <div className="flex min-w-0 flex-col gap-6 px-1 sm:px-0">
      {!hasAnyResult && (
        <div className="text-center py-8 border border-dashed border-border rounded-xl">
          <MdiIcon name={"search_off"} size={32} className="text-text-muted mb-2" />
      ...
      <div className="flex flex-col gap-4">
        ...
        anthropicCompatibleProviders.length === 0 ? (
          <div className="flex items-center justify-center gap-2 py-2 border border-dashed border-border rounded-xl text-text-muted text-sm">
            <MdiIcon name="extension" size={18} />
      ...
      <div className="flex flex-col gap-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:items-center">
            ...
            >
              <MdiIcon name="play_arrow" size={14} spin={testingMode === "oauth"} />
      ...
      <div className="flex flex-col gap-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          >
            <MdiIcon name="play_arrow" size={14} spin={testingMode === "free"} />
      ...
      <div className="flex flex-col gap-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          >
            <MdiIcon name="play_arrow" size={14} spin={testingMode === "apikey"} />
        ...
        {!isApikeySearching && !showAllApikey && hiddenApikeyCount > 0 && (
          ...
          >
            <MdiIcon name="expand_more" size={16} />
      ...
      {testResults && (
        <div
          className={mainPanelOverlayClass("flex items-start justify-center px-3 pt-[6vh] sm:pt-[10vh]")}
                ...
                {allDisabled ? (
                  <Badge variant="default" size="sm">
                    <span className="flex items-center gap-1">
                      <MdiIcon name={"pause_circle"} size={12} />
                      Disabled
                    </span>
                  </Badge>
                ) : isNoAuth ? (
...
function ApiKeyProviderCard({
  ...
  const getIconPath = () => {
    if ((isCompatible || isAnthropicCompatible) && provider.baseUrl) return null;
          ...
          <div className="flex min-w-0 items-center gap-3">
            ...
            >
              <ProviderIcon
                ...
                completionBaseUrl={provider.baseUrl}
  ...
  if (results.error && !results.results) {
    return (
      <div className="text-center py-6">
        <MdiIcon name={"error"} size={32} className="text-red-500 mb-2 block" />
