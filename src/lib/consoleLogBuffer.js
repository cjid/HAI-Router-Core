...
import { inferConsoleLogLevel, shouldSkipConsoleLine } from "@/shared/utils/consoleLogLevel.js";
...
function toLogLine(level, args) {
  ...
  const tag = inferConsoleLogLevel(level, body);
  return `[${tag}] ${body}`;
}
...
function appendLine(line) {
  if (shouldSkipConsoleLine(line)) return;
