...
function startBackgroundTokenRefreshFromCustomServer() {
  ...
  import(pathToFileURL(modPath).href)
    .then((m) => {
      ...
      const shutdownConcurrency = () => {
        import(pathToFileURL(path.join(__dirname, "open-sse", "concurrency", "index.js")).href)
          .then((c) => c.shutdownLanes?.())
          .catch(() => {});
      };
      const onStop = () => {
        stop();
        shutdownConcurrency();
        import(pathToFileURL(path.join(__dirname, "src", "lib", "goEngine", "goEngineManager.js")).href)
          .then((m) => m.shutdownGoEngine?.())
          .catch(() => {});
      };
